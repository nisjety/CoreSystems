...
- **Processor:** Velion `[legal entity, org. no., address]` ("Velion")
...
## 2. Roles and scope
2.1 The Customer is the **Controller**; Velion is the **Processor**, processing
Personal Data only on the Customer's behalf to provide the Velion AI agent
platform.
...
(consistent with Velion's [ROPA](./ropa.md)).
...
- **(a) Documented instructions.** Process Personal Data only on the Customer's
  ...
  (in which case Velion informs the Customer unless legally prohibited). Velion
...
listed in Velion's [Subprocessor List](./subprocessor-list.md), as in force from
time to time.
4.2 Velion gives the Customer **prior notice** of any intended addition or
replacement of a Subprocessor, allowing the Customer to **object** on reasonable
data-protection grounds within `[30]` days.
4.3 Velion imposes on each Subprocessor, by written contract, data-protection
...
5.1 Velion processes Personal Data in the **EU/EEA (Azure OpenAI Sweden Central)**
...
of storage location; Velion does not warrant immunity from lawful foreign-government
access.
...
Taking into account the nature of the processing, Velion assists the Customer by
...
decision-making (Art. 22)**, supported by Velion's human Approve/Reject gate and
durable run-history. Velion forwards any request received directly from a Data
...
8.1 Velion notifies the Customer **without undue delay** and in any event within
...
8.3 Velion assists the Customer in meeting its own Art. 33 (Datatilsynet, 72-hour)
and Art. 34 (Data Subject) obligations. Velion does **not** notify Datatilsynet on
the Customer's behalf unless expressly instructed.

## 9. Deletion and return on termination (Art. 28(3)(g))
9.1 On termination, and at the Customer's choice, Velion **deletes or returns** all
...
10.1 Velion makes available information reasonably necessary to demonstrate
