...
Verevon is an AI agent platform. We process your prompts, files, and the content
...
| Application data, conversations, run-history, audit log | EU/EEA Verevon-controlled stores (PostgreSQL, object storage, vector store) | Multi-tenant with strict per-organization isolation. |
...
|------|--------------------|--------------------------|---------------------|
| **Prompts** (what you/your agents send the model) | Sent to Azure OpenAI in Sweden Central to generate a response. | **No.** | Not retained by the model provider (ZDR). Retained by Verevon only as part of conversation / run-history per §3. |
...
Under Verevon's Zero Data Retention configuration, **Microsoft / Azure OpenAI does
not use your prompts or completions to train or improve its models, and does not
retain them beyond serving the request.** Verevon itself does not train or
...
Verevon agents can run multi-step, multi-tool tasks. To satisfy your obligations
around automated decision-making (GDPR Art. 22), Verevon provides:
...
Verevon is a tool you operate; **you (the controller) remain responsible** for the
...
*Questions about this policy should go to your Verevon account contact. This is a
