...
| **3 — Profile/cookie management** | UI for Quarry profiles; persistent vs ephemeral selection; enforce ZDR no-persistence. | **partial — ZDR enforcement done, full profile-management UI deferred** (2026-07-08). Recon: Quarry's `ProfileStore` (`quarry-edge/src/profile_routes.rs`, backed by 3 implementations — `s3_profile_store.rs`, `cached_profile_store.rs`, `postgres_profile_store.rs` — in `quarry-runtime`) stores only an opaque `profile_id` → `SessionSnapshot` (cookies/storage/UA/viewport/locale/timezone); there is **no name or scope field anywhere in the store, trait, or wire schema** — `SaveProfileRequest`/`SaveProfileResponse`/`ListProfilesResponse` are id-only. The SPA's `BrowserProfileScope = ephemeral\|user_private\|org_shared\|run_scoped` type is **not** a stored value — the gateway (`browser.rs`) derives just two of those four (`user_private` vs `run_scoped`) from a single `persistent_profile` boolean at session-create time; `ephemeral`/`org_shared` are unimplemented. Adding real name/scope storage would mean changing the `ProfileStore` trait signature and all 3 backends (including a Postgres migration) — genuinely not a trivial addition, so per this pass's mandate it is **noted as a scope gap** rather than forced through under time pressure; a full create/name/scope/inspect/delete profile-management popover is deferred to a Phase-3 continuation. What **did** land this pass, because it is safety-critical and was explicitly required regardless of the UI's maturity: (1) `CreateSessionBody` on the Velion gateway (`browser.rs`) gained a real `zdr: bool` field — previously `create_session` **hardcoded `let zdr = false`** unconditionally (a real, pre-existing gap: no session, ZDR or not, could ever actually be marked ZDR, despite Phase 2's ZDR-propagation logic already trusting `BrowserRunMetadata.zdr` as if it were meaningful); (2) a new pure function `reject_zdr_persistent_profile(zdr, profile_id, persistent_profile)` rejects — server-side, before any upstream Quarry call — any `POST /api/v1/browser/sessions` that requests `zdr: true` together with a `profileId` or `persistentProfile: true`, returning `400 zdr_persistent_profile_forbidden`; a client can never race this by supplying a stale/forged flag combination. (3) Four regression tests added and passing (`zdr_session_is_rejected_when_requesting_a_persistent_profile_by_id`, `..._by_flag`, `zdr_session_without_a_persistent_profile_request_is_allowed`, `non_zdr_session_may_use_a_persistent_profile`) in `apps/Frontend Plane/velionv3/apps/gateway/src/domains/browser.rs`. (4) SPA `CreateBrowserSessionRequest` (`browser-client.ts`) gained the matching optional `zdr` field so the client surface is honest about what the server now accepts, though no SPA UI control sets it yet (`tryCreateBrowserSession` in `KnowledgeComposer.tsx` never passes `zdr` today — this is the concrete first task for the Phase-3 continuation, alongside the profile-management popover itself). Verified: gateway `cargo clippy -D warnings` and `cargo test` (178 passed/0 failed, including the 4 new tests) both green; SPA `pnpm typecheck` green. Live-verified against the rebuilt gateway container as `local@velion.dev`: `POST /api/v1/browser/sessions` with `{zdr: true, persistentProfile: true}` returned `400 zdr_persistent_profile_forbidden` without any upstream Quarry call; the same request with `zdr: true` alone (no profile) succeeded normally. Commit: `9307af55`. |
