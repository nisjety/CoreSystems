...
- `apps/Frontend Plane/velionv2` is historical/reference unless explicitly targeted.

This is the single consolidated runtime map for the currently active CoreSystem planes. It combines the authority model, current Velion v3 gateway wiring, compose/runtime evidence, and remaining integration-proof gaps.
...
| Gateway reachability to planes | Confirmed | `velion-gateway-rs` can resolve and reach Control, Data, Ingestion, Model, and Application service names over `inter-plane-bus`. |
...
| Authenticated user E2E | Not proven | Velion gateway signup works, but signin is blocked by email verification and no Playwright/authenticated browser harness exists. |
| Cross-plane policy blockers | Open | Data documents-api Control consultation and Velion onboarding-through-Quarry-edge are still open. |
...
Frontend presents and normalizes access through Velion v3.
```

Non-negotiable rules:

1. No direct database crossing between planes.
2. Frontend/browser traffic enters backend planes through the Velion v3 gateway.
...
```mermaid
flowchart TB
  user["User / operator"] --> spa["Frontend Plane: Velion v3 Solid/Vite SPA"]
  spa --> gw["Frontend Plane: velion-gateway-rs"]
...
|---|---|---|---|
| Frontend Plane | Human-facing Velion v3 UI and same-origin gateway/BFF. | `apps/Frontend Plane/velionv3/docker-compose.yml` | `velionv3`, `velion-gateway-rs`, `velion-nats` |
...
## Velion v3 Gateway Wiring

Velion v3 has three runtime surfaces:
...
| `apps/velion-web` Next.js app | Separate web app under Velion v3; ownership still needs explicit documentation. |
...
| `docker compose config --quiet` for six main stacks | Pass | Frontend Velion v3, Control, Data v2, Ingestion, Application, and Model compose files validate. |
...
| Frontend | `http://localhost:3185/health` | 200, `velion-gateway-rs`. |
...
From `velion-gateway-rs`, the following service names resolved and returned 200:
...
| Velion auth gateway probe | throwaway signup/signin curl flow | Partial | Signup returned 200; signin returned `EMAIL_NOT_VERIFIED`; `/api/v1/me` and session-context remain 401 without a verified session. |
| Quarry edge onboarding probe | host curl/source inspection | Open | Velion onboarding still calls `quarry-control`; edge `/v1/jobs` returns 401 without edge auth/HMAC while control `/v1/jobs` lists jobs under rollout mode. |
...
| L2 network reachability | Cross-plane service names resolve from the containers that need them. | Confirmed from Velion gateway and Model gateway. |
...
| P1 | No authenticated Velion browser E2E ran. | No Playwright dependency/config exists; gateway signup succeeds but signin blocks on `EMAIL_NOT_VERIFIED`, leaving protected routes 401. | Add a seeded verified test account or test-only verified-session fixture, then add Playwright smoke coverage across gateway route families. |
...
2. Add a Velion gateway smoke suite that runs from inside `velion-gateway-rs` and checks route families, not just service health.
3. Add a seeded verified auth fixture and an authenticated Velion Playwright smoke that proves at least one user journey through every active plane.
4. Implement/prove documents-api Control consultation and tenant isolation.
5. Move/prove Velion onboarding ingestion through `quarry-edge`.
