...
| Long-term / semantic memory | ✅ **[was 🟡; CORRECTED twice — 2026-08-22 and again 2026-08-26]** `letta-bridge` has a real `Store` seam with THREE implementations (`agentmemory` semantic client, `pgstore` Postgres-lexical, `memstore` in-memory fallback), honest degraded-readiness reporting (`DEGRADED_LEXICAL_FALLBACK` / `DEGRADED_SEMANTIC_UNVERIFIED`), memory prefetch wired on the live turn path (`model-gateway/grpc.rs`), and memory read/write exposed as model-callable tools (`tool_loop.rs`). **[RE-VERIFIED 2026-08-26 — the lifecycle gap does not survive a call-site check]** every item §7.9 listed is now closed, exists under our own name, or does not transfer: SSE chat prefetch is wired (400 ms bound, `is_trivial_prompt` skip with 52 entries incl. 22 Norwegian); the model-callable write is reachable in execution-core and its capability is **attested live**; `on_pre_compress`, `on_delegation` and `recall_status` exist by name; session-end extraction is `dreaming::dream_once`; `system_prompt_block` is the `"Relevant memory:"` injection on **both** paths; and "no memory context segment is produced anywhere" was a grep for a segment *kind* named `memory` — `bucket_memory_segments` fans memory into five typed fields (`policy_/workspace_/agent_/user_/episodic_segments`). Genuinely non-transferable: `backup_paths` (Hermes is file-backed, we are Postgres) and `flush_pending` + durability-classed background writes — ours are **awaited**, so there is no pending queue to flush, pinned by `a_memory_write_is_awaited_not_backgrounded` so the analysis cannot rot silently | ✅ `memdir/` + auto-extraction + team sync | 🟡 read-only cross-session query only, no vector memory shipped | ✅ `MemoryProvider` ABC, 8 real backends |
...
| Read on the **SSE chat path** (the browser product) | ✅ **wired** (re-verified 2026-08-26) | `fetch_chat_memory_context`, `sse.rs`; 400 ms bound + trivial-prompt skip |
| Memory as a context-assembly segment | ✅ **it always was — the grep was for the wrong thing** (2026-08-26) | `bucket_memory_segments` (`session-core/grpc.rs:200`) → `policy_/workspace_/agent_/user_/episodic_segments`. Searching for a segment *kind* named `memory` finds nothing because memory is bucketed into five typed fields |
| **Model-callable memory write** | ✅ **reachable and attested live** (2026-08-26) | execution-core offers and dispatches `save_memory`/`recall_memory`; their capabilities had no durable row and no attestor — migration 0014 seeds both and the health reporter attests them from a session-core probe, verified `available` on the running stack |
