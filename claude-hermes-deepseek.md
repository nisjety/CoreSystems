...
| Elicitation instruction for un-inventable arguments | ✅ **both loops** (2026-08-26) — `SNIPPET_USER_SUPPLIED_ARGS` is now byte-identical in model-gateway and execution-core, pinned by a decoded-string contract test (raw source comparison would assert formatting, not words). Agent loop measured +18.3 pp (p=0.016, zero under-calling regression at n=60); chat measured only +4/20, because there the snippet is the ONLY tool guidance in the stack while the agent loop's rides a full preamble — so on chat the grounding gate carries correctness and the snippet saves the refusal round-trip. Do not quote the agent number for chat | 🟡 (per-tool description prose) | ❌ | ❌ |
