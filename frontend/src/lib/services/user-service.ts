...
class UserServiceAPI {
  ...
  async getCurrentUserProfile(email: string, userId?: string): Promise<UserProfile | null> {
    ...
    try {
      const payload = await apiClient.get<unknown>(
        this.getUserEndpoint(`/users/me?email=${encodeURIComponent(email)}`),
        options
      )
      return this.normalizeCurrentUserProfile(payload)
    } catch (error) {
      const message = error instanceof Error ? error.message.toLowerCase() : ''
      if (message.includes('not found') || message.includes('profile')) {
        return null
      }
      throw error
    }
  ...
  async getCurrentUser(email: string, userId?: string): Promise<User> {
    const options = userId
      ...
      : undefined
    const payload = await apiClient.get<unknown>(
      this.getUserEndpoint(`/users/current?email=${encodeURIComponent(email)}`),
      options
    )
  ...
  async markOnboardingComplete(email: string, userId?: string): Promise<void> {
    ...
    return apiClient.post<void>(
      this.getUserEndpoint(`/users/onboarding/complete?email=${encodeURIComponent(email)}`),
      {},
      options
    )
  }
