...
async function handler(
  ...
  const targetPath = `/api/auth/${authPath}${queryString}`;
  ...
  try {
    ...
    const { response, targetUrl } = await fetchFromAuthService(targetPath, {
      method: request.method,
      headers: forwardHeaders,
      body,
      redirect: 'manual',
      retryOn5xx: allowDevFallback && authPath === 'get-session',
    });
    ...
    if (allowDevFallback && authPath === 'get-session' && response.status >= 500) {
    ...
    if (allowDevFallback && authPath === 'get-session') {
