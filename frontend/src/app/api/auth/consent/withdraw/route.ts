...
export async function POST(request: NextRequest) {
  ...
  const targetUrl = `${backendUrl}/api/v2/auth/consent/withdraw`;

  console.log(`🔄 Proxying consent withdraw request: POST -> ${targetUrl}`);
  ...
  try {
    ...
    console.error('❌ Consent withdraw proxy error:', error);
    return NextResponse.json(
      {
        error: 'Consent withdraw proxy failed',
