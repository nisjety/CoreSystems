...
// Norwegian loading spinner component
...
export function AuthGuard({
  ...
  const [validationError, setValidationError] = useState<GuardReason | null>(null);
  const [hasMinLoadingPassed, setHasMinLoadingPassed] = useState(false);
  const [customValidationResult, setCustomValidationResult] = useState<boolean | null>(null);
  ...
  setValidationError(null);
        ...
        if (!validationChecks.isValid && validationChecks.reason) {
          setValidationError(validationChecks.reason);
          setAuthState('unauthenticated');
          if (onAccessDenied) {
  ...
  setAuthState('error');
  setValidationError({ key: err instanceof Error ? err.message : 'authErrorMessage' });
      }
    };
  ...
  useEffect(() => {
    ...
    const runCustomValidation = async () => {
      try {
        ...
        setCustomValidationResult(false);
        if (onAccessDenied) {
...
export const KreverAuth = ({ children, ...props }: Omit<AuthGuardProps, 'allowedStates'>) => (
  <AuthGuard allowedStates={['authenticated']} {...props}>
    {children}
  </AuthGuard>
);

export const KreverGjest = ({ children, ...props }: Omit<AuthGuardProps, 'allowedStates'>) => (
  <AuthGuard allowedStates={['unauthenticated']} {...props}>
    {children}
  </AuthGuard>
);

export const KreverEpostBekreftelse = ({ children, ...props }: Omit<AuthGuardProps, 'requireEmailVerification'>) => (
  <AuthGuard requireEmailVerification={true} {...props}>
    {children}
  </AuthGuard>
);

export const KreverTofaktor = ({ children, ...props }: Omit<AuthGuardProps, 'requireTwoFactor'>) => (
