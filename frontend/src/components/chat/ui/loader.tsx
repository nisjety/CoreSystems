...
export const LoaderOne = () => {
  const transition = (x: number) => ({ duration: 1, repeat: Infinity, repeatType: 'loop' as const, delay: x * 0.2 });
  return (
...
export const LoaderTwo = () => {
  const transition = (x: number) => ({ duration: 2, repeat: Infinity, repeatType: 'loop' as const, delay: x * 0.2 });
  return (
...
export const LoaderFour = ({ text = "Loading..." }: { text?: string }) => {
  return (
    <div className="relative font-bold text-black [perspective:1000px] dark:text-white">
      <motion.span
  animate={{ rotate: [0, -5, 0], scaleX: [1, 1.2, 1] }}
        transition={{
          duration: 0.05,
          repeat: Infinity,
          repeatType: "reverse",
