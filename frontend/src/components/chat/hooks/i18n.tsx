...
import { useGlobalLanguage } from '@/components/shared/contexts/GlobalLanguageContext';
...
export const AppI18nProvider: React.FC<{ children: React.ReactNode; initialLocale?: Locale }> = ({ children, initialLocale = 'en' }) => {
	...
	useEffect(() => {
		const currentChatLocale = globalLang.getChatLocale();
		if (currentChatLocale !== locale) {
			setLocale(currentChatLocale);
		}
	}, [globalLang, locale]);
...
export const useI18n = (_initialLocale?: Locale) => {
	...
	// Sync fallback with global language
	useEffect(() => {
		const currentChatLocale = globalLang.getChatLocale();
		if (currentChatLocale !== fallbackLocale) {
			setFallbackLocale(currentChatLocale);
		}
	}, [globalLang, fallbackLocale]);
