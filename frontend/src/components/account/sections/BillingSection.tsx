...
import { Loader2, CreditCard, AlertCircle, CheckCircle2, Clock, Zap } from 'lucide-react'
import { useCurrentOrganization, useOrgBilling, useOrgQuotas } from '../hooks/useAccount'
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="font-inter text-[12px] text-[#4A4A48] capitalize">
  ...
  return (
    <section id="billing" className="scroll-mt-6">
      ...
      <p className="mb-6 font-inter text-[13px] text-[#9B9691]">
        Your current plan and usage for{' '}
        <strong className="font-medium text-[#1C1C1A]">{org?.name ?? 'your organisation'}</strong>.
      </p>
          ...
          {!org && (
            <div className="rounded-xl border border-dashed border-[#D8D2C6] px-4 py-6 text-center">
              <p className="font-inter text-[13px] text-[#9B9691]">
                No organisation found. Create one to manage billing.
              </p>
            </div>
          )}
        </div>
