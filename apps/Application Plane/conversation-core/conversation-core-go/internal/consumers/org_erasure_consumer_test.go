...
// so the consumer's ack-vs-retry policy can be asserted.
...
// authority — it must ack without ever calling the purger.
func TestOrgErasure_NonOrganizationSubject_AcksWithoutPurging(t *testing.T) {
	purger := &fakeOrgPurger{}
	c := &OrgErasureConsumer{purger: purger}
	ev := orgErasureEvent{SubjectType: "user", SubjectID: "user-1", OrgID: "org-1"}

	if got := c.process(context.Background(), ev); got != outcomeAck {
		t.Fatalf("outcome = %v, want outcomeAck for a non-organization erasure", got)
	}
	if purger.count() != 0 {
		t.Fatalf("purge called for a non-organization erasure event: %v", purger.orgIDs)
	}
}
