...
Last verified: 2026-08-17 CEST. This status separates checked source from the running workload. See `docs/core-research/plane-audit-2026-07-13.md` for the historical audit; the dated notification source proof below is newer than that audit and is still not a deployment claim.
...
| notification-core | Existing container is healthy; the running image revision is not proven to contain the current delivery source | org-scoped typed recipient, signed delegation, membership gating, workflow allowlists, ZDR metadata-only ledger, explicit disabled/Novu mode, lease-fenced delivery attempt ledger/worker, config-gated provider callback route/verifier, and a content-free feed-projection outbox/projector with callback-before-projection reconciliation | source/disposable PostgreSQL proof covers two-worker lease fencing, duplicate callbacks, callback ordering, and idempotent Activity/Inbox replay; deployment, provider/ZDR attestation, multi-replica replay, and candidate/rollback evidence remain open |
...
5. Notification source now has an organization boundary and typed user recipient, but no signed/revisioned Control-authority writer or safe backfill exists; the secure source therefore denies legitimate access. A content-free, lease-fenced delivery attempt ledger, opt-in worker, config-gated HMAC/replay callback route, and callback contract are source-tested. Submission atomically creates a content-free feed-projection obligation, callback receipts reconcile before or after projection, and a real disposable-Postgres test proves two-worker lease fencing and idempotent Activity/Inbox replay. Provider/ZDR attestation, deployment of the changed image/migrations, and durable multi-replica candidate replay remain open.
...
- Notification: `go test -race -cover ./...` passes; delegation 81.4%, notification 59.1%, HTTP 32.6%, database 41.0%. The focused real-migration disposable-Postgres delivery/feed proof also passes with an explicitly supplied throwaway test URL; this is integration evidence, not deployed-provider or candidate evidence.
