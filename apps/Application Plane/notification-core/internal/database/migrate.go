...
const (
	notificationMigrationTableName      = "notification_core_schema_migrations"
	createNotificationMigrationTableSQL = `
CREATE TABLE IF NOT EXISTS notification_core_schema_migrations (
	name TEXT PRIMARY KEY,
	applied_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
)`
	notificationMigrationExistsSQL = `
SELECT EXISTS(
	SELECT 1
	FROM notification_core_schema_migrations
	WHERE name = $1
)`
	recordNotificationMigrationSQL = `
INSERT INTO notification_core_schema_migrations (name)
VALUES ($1)
...
func runMigrations(ctx context.Context, executor migrationExecutor, dir string) error {
	...
	if _, err := executor.Exec(ctx, createNotificationMigrationTableSQL); err != nil {
		return fmt.Errorf("ensure %s: %w", notificationMigrationTableName, err)
	}

	for _, file := range files {
		var applied bool
		if err := executor.QueryRow(ctx, notificationMigrationExistsSQL, file.name).Scan(&applied); err != nil {
...
func applyMigration(ctx context.Context, executor migrationExecutor, file migrationFile) error {
	...
	if _, err := tx.Exec(ctx, recordNotificationMigrationSQL, file.name); err != nil {
