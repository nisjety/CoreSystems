...
| **zammad-foundation** | Not a runtime service | Deployment/bootstrap package for a self-hosted Zammad (own isolated postgres, opt-in separate compose, NOT running). Zero Application-Plane consumers (conversation-core owns v3 Inbox), but NOT dead — verevonv3 gateway Agent Console, Ingestion support-worker, and verevon-web support UI target `zammad-railsserver:3000` (all degrading gracefully, tokens empty). Footguns: example `ZAMMAD_EXPOSE_PORT=8088` collides with model-plane-letta-bridge; example webhook config ships `active:true` pointing at a nonexistent `integration-core` receiver. |
...
| P2 | Application gateway naming is confusing beside the current Verevon v3 Frontend gateway. | Existing core research includes `verevon-gateway-rs`; current Verevon v3 also has `apps/Frontend Plane/verevonv3/apps/gateway`. | Clarify whether Application `verevon-gateway-rs` is transitional, legacy, or still live for onboarding. |
...
2. Clarify Application `verevon-gateway-rs` vs Frontend Verevon v3 gateway ownership.
