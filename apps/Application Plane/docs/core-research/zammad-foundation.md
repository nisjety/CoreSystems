...
Stock-lean Zammad 7.0.0-0042 (`ghcr.io/zammad/zammad`) on its own `zammad-net`, attaching app services to external `velion-net`:
...
- `zammad-railsserver` / `-scheduler` / `-websocket` / `-nginx` are on `velion-net`, so other planes reach the Zammad REST API internally at `http://zammad-railsserver:3000`.
...
- [source-only] **`velion` (older Next.js web app, not the v3 SPA)** — full support UI: `src/lib/clients/zammad-client.ts`, `src/lib/hooks/useZammad.ts`, and ~10 `src/app/api/support/*` routes (tickets, articles, agents, groups, macros, reports, summarize, sentiment, quick-replies).
- [source-only] **`velionv2` (deprecated)** — `src/app/api/support/_lib/zammad.ts`.
