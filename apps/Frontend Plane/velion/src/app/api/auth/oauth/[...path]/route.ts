...
async function handler(
  ...
  // Prepend 'oauth/' so the full path is correctly forwarded
  const authPath = `oauth/${path.join('/')}`;
  const backendUrl = getAuthServiceUrl();
  const searchParams = request.nextUrl.searchParams.toString();
  const queryString = searchParams ? `?${searchParams}` : '';
  const targetUrl = `${backendUrl}/api/auth/${authPath}${queryString}`;

  console.log(`🔄 Proxying oauth request: ${request.method} ${authPath} -> ${targetUrl}`);
  ...
  try {
    ...
    console.error(`❌ Failed to proxy oauth request ${authPath}:`, error);
