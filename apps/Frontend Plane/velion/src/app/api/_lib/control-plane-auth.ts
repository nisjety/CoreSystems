...
/**
 * G15: stable per-request correlation id for cross-plane log correlation.
 * Honours an inbound `x-correlation-id` if present, otherwise mints a UUIDv4.
 * Cached on the NextRequest so every downstream call in the same request
 * carries the same id.
 */
export function getCorrelationId(request: NextRequest): string {
  ...
  const id = inbound && inbound.length > 0 && inbound.length <= 128
    ? inbound
    : crypto.randomUUID()
...
export const getAuthServiceUrl = () =>
  trimRightSlash(process.env.AUTH_SERVICE_URL || 'http://auth-service:3011')

export const getUserServiceUrl = () =>
  trimRightSlash(process.env.USER_SERVICE_URL || 'http://user-core:3012')
...
async function fetchSession(request: NextRequest): Promise<ControlPlaneSession | null> {
  // G30 v3: uses Better Auth's native `/api/auth/get-session` (GET) rather
  // than the custom oRPC `/api/v2/auth/getSession` wrapper, which had shown
  // intermittent "not authenticated" responses for cookies that Better Auth's
  // own validation accepts.
