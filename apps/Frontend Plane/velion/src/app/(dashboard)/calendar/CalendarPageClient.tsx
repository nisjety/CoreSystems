...
export function CalendarPageClient({
  ...
  const { data = initialEvents } = useCalendarEvents({
    enabled: true,
  ...
  const selectedItem = useMemo<SidebarDetailItem | null>(() => {
    ...
    return selectedEvent ? { kind: 'event', item: selectedEvent } : null;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-[#F5F3EE]">
      ...
      {selectedItem && (
        <SidebarItemDetailDrawer item={selectedItem} onClose={() => setSelectedEventId(null)} />
      )}
    </div>
  );
}
