...
const PlannerEditor = dynamic(() => import('./PlannerEditor').then((mod) => mod.PlannerEditor), {
  ssr: false,
  loading: () => (
...
const PlannerSidebar = dynamic(() => import('./PlannerSidebar').then((mod) => mod.PlannerSidebar), {
  ssr: false,
  loading: () => (
    <div className="hidden w-[320px] shrink-0 rounded-[32px] border border-stone-200/80 bg-[rgba(252,249,244,0.97)] shadow-[0_28px_90px_rgba(110,90,60,0.10)] lg:block" />
  ),
});

const PlannerToolbar = dynamic(() => import('./PlannerToolbar').then((mod) => mod.PlannerToolbar), {
  ssr: false,
});
...
interface PlannerWorkspaceClientProps {
  initialWorkspace: PlannerWorkspaceResolution | null;
}

export function PlannerWorkspaceClient({ initialWorkspace }: PlannerWorkspaceClientProps) {
  const seededInitialDocRef = useRef(false);
  const viewedDocIdsRef = useRef<Set<string>>(new Set());
  const workspaceId = initialWorkspace?.workspaceId ?? 'planner-anonymous';
  const workspaceState = {
    isLoading: false,
    error: initialWorkspace ? null : 'Failed to prepare planner workspace',
    scope: initialWorkspace?.scope ?? 'anonymous',
  };
  ...
  const selectedDocId = useMemo(
    () =>
      activeDocuments.some((document) => document.id === requestedDocId)
        ? requestedDocId
        : activeDocuments[0]?.id ?? null,
    [activeDocuments, requestedDocId],
  );

  const selectedDocument = useMemo(
    () => activeDocuments.find((document) => document.id === selectedDocId) ?? null,
  ...
  useEffect(() => {
    ...
    if (viewedDocIdsRef.current.has(selectedDocId)) {
      return;
    }
    viewedDocIdsRef.current.add(selectedDocId);
          ...
          {workspaceState.isLoading ? (
            <div className="flex h-full min-h-0 flex-1 items-center justify-center rounded-[32px] border border-dashed border-stone-300 bg-[rgba(252,249,244,0.88)] px-8 text-center shadow-[0_28px_90px_rgba(110,90,60,0.08)]">
              <div className="flex flex-col items-center gap-4 text-stone-500">
                ...
                <p className="text-sm">Resolving your planner workspace…</p>
              </div>
            </div>
          ) : showCanvas ? (
            <PlannerProvider
