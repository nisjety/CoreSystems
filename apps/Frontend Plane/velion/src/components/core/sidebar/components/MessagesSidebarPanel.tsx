...
export function MessagesSidebarPanel({
  ...
  const {
    state: { sessions: liveSessions, isLoading, error },
    selectSession,
    startNewChat,
  } = useChat();
  ...
  const sessions = React.useMemo(() => {
    return liveSessions.filter((session) => (session.messageCount ?? 0) > 0);
  }, [liveSessions]);

  // Latch "we've ever loaded sessions" so transient `isLoading` blips
  // (Convex WebSocket reconnects, actor refetches) don't flash the
  // sidebar back to skeleton placeholders. Once we have a session
  // list, render it; the underlying ChatProvider already keeps the
  // last-good Convex payload visible during reconnects.
  const hasEverLoadedRef = React.useRef(false);
  if (liveSessions.length > 0) {
    hasEverLoadedRef.current = true;
  }
  const showSkeleton = isLoading && !hasEverLoadedRef.current;
