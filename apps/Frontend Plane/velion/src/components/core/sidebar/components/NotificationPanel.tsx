...
interface SupportNotificationFeed {
...
function formatSupportEventType(eventType: string): string {
  return eventType
    .replace(/_/g, ' ')
    .replace(/\b\w/g, (c) => c.toUpperCase())
}

function SupportNotificationsSection() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) return;
        const data = (await res.json()) as SupportNotificationFeed;
        if (!cancelled) {
          setItems(data.notifications ?? []);
        }
      } catch {
        // Degrade silently — section simply stays empty
      } finally {
  ...
  const handleClick = useCallback(
    async (item: SupportNotification) => {
      if (!item.read && !readIds.has(item.id)) {
        setReadIds((prev) => new Set([...prev, item.id]));
        fetch(`/api/support/notifications/${encodeURIComponent(item.id)}/read`, {
          method: 'POST',
        }).catch(() => undefined);
      }
      router.push('/inbox');
  ...
  if (isLoading) {
    return (
      <div className="px-3 pb-3">
        <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/36 px-1 mb-2">
          Support
        </div>
        <div className="text-xs text-black/40 px-1">Loading...</div>
      </div>
    );
  }
  ...
  return (
    <div className="px-3 pb-3">
      <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/36 px-1 mb-2 mt-1">
      ...
      <div className="space-y-2">
        {items.slice(0, 5).map((item) => {
          ...
          const typeLabel = formatSupportEventType(item.event_type);
          ...
          return (
            <div
              ...
              tabIndex={0}
            >
              <div className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center bg-[#FFF1DE] text-[#B96618]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between mb-0.5">
                  <span className={cn(
                    ...
                    !isRead ? 'text-black' : 'text-black/74',
                  )}>
                    {item.title}
                  </span>
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-medium text-[#B96618] bg-[#FFF1DE] rounded px-1.5 py-0.5">
                    {typeLabel}
                  </span>
                  {ticketId && (
                ...
                <p className="text-xs text-black/50 line-clamp-1 mt-0.5">{item.body}</p>
                {!isRead && (
                  <div className="w-1.5 h-1.5 bg-[#DD7A1F] rounded-full mt-1.5" />
                )}
              </div>
            </div>
          );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* Support notifications section — always shown below main notifications */}
        <SupportNotificationsSection />
      </div>
    </div>
  );
}