...
export function useCurrentOrganization() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const sessionContext = await profileService.getSessionContext();
      if (sessionContext?.orgId) {
        const activeOrg = orgs.find((org) => org.id === sessionContext.orgId);
        if (activeOrg) {
          return activeOrg;
        }
      }
...
function useProfileModal(options?: { onOpen?: () => void; onClose?: () => void }) {
