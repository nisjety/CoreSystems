...
import { useTeamStepState } from './use-team-step-state'
...
export function TeamStep() {
  ...
  const [state, dispatch] = useTeamStepState()
  const { loading, error, email, role, members } = state

  const addMember = () => {
    if (!email) return
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (!emailRegex.test(email)) {
      dispatch({ type: 'SET_ERROR', payload: 'Skriv inn en gyldig e-postadresse' })
      return
    }
    if (members.some(m => m.email === email)) {
      dispatch({ type: 'SET_ERROR', payload: 'Denne e-postadressen er allerede lagt til' })
      return
    }
    dispatch({ type: 'ADD_MEMBER', payload: { email, role } })
  }

  const removeMember = (emailToRemove: string) => {
    dispatch({ type: 'REMOVE_MEMBER', payload: emailToRemove })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch({ type: 'SUBMIT_ERROR', payload: err.message || 'Kunne ikke sende invitasjoner' })
    } finally {
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      dispatch({ type: 'SUBMIT_ERROR', payload: err.message || 'Kunne ikke fullføre onboarding' })
    } finally {
      dispatch({ type: 'SUBMIT_END' })
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} data-testid="onboarding-team-form" className="space-y-5">
      {/* Email + role + add row */}
      <div className="flex gap-3 items-end">
        <div className="flex-1">
          ...
          <input
            id="email"
            data-testid="onboarding-team-email"
            type="email"
            value={email}
        ...
        <div className="w-28 shrink-0">
          ...
          <select
            id="role"
            data-testid="onboarding-team-role"
            value={role}
            onChange={(e) => dispatch({ type: 'SET_ROLE', payload: e.target.value as 'admin' | 'member' | 'viewer' })}
            className={inputClass}
          >
        ...
        <div className="shrink-0">
          <button
            type="button"
            data-testid="onboarding-team-add-member"
            onClick={addMember}
      ...
      {members.length > 0 && (
        <div>
          ...
          <div data-testid="onboarding-team-members-list" className="space-y-2 max-h-52 overflow-y-auto">
            {members.map((member) => (
              <div
                key={member.email}
              ...
              >
                ...
                <button
                  type="button"
                  data-testid="onboarding-team-remove-member"
      ...
      <div className="flex justify-between pt-2">
        <button
          ...
          onClick={async () => { await onboardingService.saveCurrentStep('connect'); router.back() }}
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            type="submit"
            data-testid="onboarding-team-submit"
            disabled={loading}
