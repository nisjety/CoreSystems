...
function resolveFrontendBaseUrl(): string {
  ...
  throw new Error(
    'Missing NEXT_PUBLIC_APP_URL or APP_URL for server-side auth provider requests.',
  );
}

function resolveRequestUrl(baseUrl: string, endpoint: string): string {
...
async function readErrorMessage(response: Response): Promise<string> {
  ...
  try {
    ...
    // Fall back to a trimmed text response.
  }

  return responseText.length > 200 ? `${responseText.slice(0, 200)}...` : responseText;
}
...
class AuthProviderClient {
  ...
  constructor() {
    this.baseURL = resolveFrontendBaseUrl();
  }
  ...
  private async getSessionToken(): Promise<string | null> {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn('Auth session lookup failed; continuing with cookie-based request.', error);
      }
      return null;
    }
  }
  ...
  private async enhancedFetch(endpoint: string, options: RequestInit = {}): Promise<Response> {
    const requestUrl = resolveRequestUrl(this.baseURL, endpoint);
    ...
    const timeoutId = setTimeout(() => controller.abort(), AUTH_PROVIDER_TIMEOUT_MS);
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new Error(`Auth provider request timed out after ${AUTH_PROVIDER_TIMEOUT_MS}ms`);
      }
