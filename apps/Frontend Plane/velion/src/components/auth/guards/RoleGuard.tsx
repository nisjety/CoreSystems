...
interface RoleGuardState {
  validationState: RoleValidationState;
  errorMessage: string;
}

type RoleGuardAction =
  | { type: 'LOADING' }
  | { type: 'AUTHORIZED' }
  | { type: 'UNAUTHORIZED'; message: string }
  | { type: 'ERROR'; message: string };

function roleGuardReducer(_state: RoleGuardState, action: RoleGuardAction): RoleGuardState {
  switch (action.type) {
    case 'LOADING':
      return { validationState: 'loading', errorMessage: '' };
    case 'AUTHORIZED':
      return { validationState: 'authorized', errorMessage: '' };
    case 'UNAUTHORIZED':
      return { validationState: 'unauthorized', errorMessage: action.message };
    case 'ERROR':
      return { validationState: 'error', errorMessage: action.message };
  }
}
...
function RoleLoadingView({ locale, allowedRoles }: { locale: string; allowedRoles: string[] }) {
...
export function RoleGuard({
  ...
  allowedPermissions = EMPTY_ARRAY,
  organizationId,
  organizationRoles = EMPTY_ARRAY,
  organizationPermissions = EMPTY_ARRAY,
  roleStrategy = 'any',
  ...
  const [{ validationState, errorMessage }, dispatch] = useReducer(roleGuardReducer, { validationState: 'loading', errorMessage: '' });
    ...
    const validateAccess = async () => {
      try {
        ...
        if (!isAuthenticated || !user) {
          dispatch({ type: 'UNAUTHORIZED', message: 'authRequired' });
          if (onAccessDenied) {
        ...
        if (requireActiveAccount && (user as unknown as { status?: string }).status === 'inactive') {
          dispatch({ type: 'UNAUTHORIZED', message: 'inactiveAccount' });
          if (onAccessDenied) {
        ...
        if (!checkTimeRestrictions()) {
          dispatch({ type: 'UNAUTHORIZED', message: 'timeRestricted' });
          if (onAccessDenied) {
        ...
        if (organizationId) {
          ...
          if (!userOrg) {
            dispatch({ type: 'UNAUTHORIZED', message: 'organizationAccessRequired' });
            if (onAccessDenied) {
          ...
          if (organizationRoles.length > 0) {
            ...
            if (!hasOrgRole) {
              dispatch({ type: 'UNAUTHORIZED', message: 'organizationRoleRequired' });
              if (onAccessDenied) {
          ...
          if (organizationPermissions.length > 0) {
            ...
            if (!hasOrgPermissions) {
              dispatch({ type: 'UNAUTHORIZED', message: 'organizationPermissionsRequired' });
              if (onAccessDenied) {
        ...
        if (allowedRoles.length > 0) {
          ...
          if (!hasRole) {
            dispatch({ type: 'UNAUTHORIZED', message: 'roleRequiredGlobal' });
            if (onAccessDenied) {
        ...
        if (allowedPermissions.length > 0) {
          ...
          if (!hasPermissions) {
            dispatch({ type: 'UNAUTHORIZED', message: 'permissionsRequiredGlobal' });
            if (onAccessDenied) {
        ...
        if (customRoleValidator) {
          try {
            ...
            if (!customResult) {
              dispatch({ type: 'UNAUTHORIZED', message: 'customValidationFailed' });
              if (onAccessDenied) {
            ...
            dispatch({ type: 'UNAUTHORIZED', message: 'customValidationError' });
            if (onAccessDenied) {
        ...
        dispatch({ type: 'ERROR', message: 'roleValidationError' });
        if (onAccessDenied) {
  ...
  if (validationState === 'loading' || validationState === 'checking') {
    ...
    return <RoleLoadingView locale={locale} allowedRoles={allowedRoles} />;
  }
...
const KreverRolle = ({ role, children, ...props }: { role: string; children: React.ReactNode } & Omit<RoleGuardProps, 'allowedRoles' | 'children'>) => (
  <RoleGuard allowedRoles={[role]} {...props}>
    {children}
  </RoleGuard>
);

const KreverTillatelse = ({ permission, children, ...props }: { permission: string; children: React.ReactNode } & Omit<RoleGuardProps, 'allowedPermissions' | 'children'>) => (
  <RoleGuard allowedPermissions={[permission]} {...props}>
    {children}
  </RoleGuard>
);

const KreverAdmin = ({ children, ...props }: Omit<RoleGuardProps, 'allowedRoles'>) => (
  <RoleGuard allowedRoles={['admin']} {...props}>
    {children}
  </RoleGuard>
);

const KreverLeder = ({ children, ...props }: Omit<RoleGuardProps, 'allowedRoles'>) => (
  <RoleGuard allowedRoles={['admin', 'leder']} {...props}>
    {children}
  </RoleGuard>
);

const KreverOrganisasjonsrolle = ({ 
  organizationId, 
  role, 
  children, 
  ...props 
    ...
    <div>{authT.guard('organizationsLabel')}: {((user as unknown as AuthUser).organizations || []).length}</div>
      ...
      <div className="text-xs font-medium mb-1">{authT.guard('organizationsHeader')}</div>
            {(user as unknown as AuthUser).organizations!.map((org, index) => (
              <div key={org.slug} className="text-xs">
