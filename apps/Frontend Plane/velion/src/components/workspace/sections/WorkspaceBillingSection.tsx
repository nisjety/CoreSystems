...
function PlanBadge({ plan }: { plan: string }) {
  const styles: Record<string, string> = {
    ...
    trial:      'bg-[#F4F1EB] text-[#4A4A48] border-[#D8D2C6]',
    hobby:      'bg-[#FFF7ED] text-[#C2410C] border-[#FED7AA]',
    standard:   'bg-[#FFF7ED] text-[#C2410C] border-[#FED7AA]',
...
function QuotaBar({ quota }: { quota: OrgQuota }) {
  const isUnlimited = quota.quotaLimit < 0
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="font-inter text-[11px] text-[#9B9691]">
          {quota.quotaValue.toLocaleString()} / {isUnlimited ? 'Unlimited' : quota.quotaLimit.toLocaleString()}
        </span>
      </div>
...
export function WorkspaceBillingSection() {
  ...
  const effectivePlan = billing?.plan ?? org?.plan ?? 'free'
