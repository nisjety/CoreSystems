...
import { VerevonComposerPreview } from "@/components/ui/VerevonComposerPreview";
...
const mediaLayers: MediaLayer[] = [
	...
	{
		kind: "image",
		src: "/verevon-product-shots/chat-agent-steps.png",
...
const copyPanels = [
	{
		body: "Verevon kjører chat, søk og agentoppgaver i EU som standard, uten at promptene dine lagres hos modell-leverandøren.",
		cta: undefined,
		align: "left",
	},
	{
		body: "Når Verevon vil sende et svar, publisere noe eller bruke et risikofylt verktøy, stopper det for Godkjenn eller Avvis.",
		cta: "Tillitssenter",
		align: "left",
	},
	{
		body: "I Verevon tillitssenter ser du hvilke kilder AI-en brukte, hvilke data som ble berørt, og at norske selskaper kobles mot Enhetsregisteret.",
		cta: undefined,
		align: "center",
	},
];
...
const workflowCards: WorkflowCard[] = [
	{
		href: "#kunnskap",
		image: "/verevon-product-shots/chat-draft-answer-top.png",
		kicker: "Svar",
		link: "Skriv utkast",
	...
	{
		href: "#plattform",
		image: "/verevon-product-shots/dashboard-composer-prompt-state.png",
		kicker: "Kunnskap",
		link: "Samle kilder",
	...
	{
		href: "/trust",
		image: "/verevon-product-shots/inbox-empty-workspace.png",
		kicker: "Spor",
...
function CopyPanel({
	...
	return (
		<div
			className={[
				"absolute left-0 top-0 max-w-[680px] text-verevon-j-text/68",
...
function FeaturePromptComposer() {
	return (
		<div
			...
			data-feature-prompt=""
		>
			<VerevonComposerPreview animateCharacters prompt={promptText} />
		</div>
	);
}
	...
	return (
		...
		>
			<div className="flex h-[15px] select-none items-center justify-between font-protokoll text-[10px] leading-none text-verevon-j-text/42">
				...
				<span>Verevon-resultat</span>
			</div>

			<div className="relative aspect-[3/4] overflow-hidden border border-verevon-j-text/8 bg-white/55 shadow-[0_10px_28px_rgba(23,23,23,0.035)]">
				<Image
					alt=""
				...
				<div className="absolute bottom-4 left-4 right-4">
					<h3 className="m-0 font-arbeit text-[clamp(1.45rem,2vw,2.2rem)] font-light leading-[1.04] tracking-[-0.055em] text-verevon-j-text">
						{card.title}
					</h3>
				</div>
			</div>

			<p className="m-0 min-h-[4.2em] font-protokoll text-[clamp(0.92rem,0.92vw,1.02rem)] font-light leading-[1.38] text-verevon-text-muted/90">
				{card.text}
			</p>

			<ArrowButtonLabel className="text-verevon-j-text/62">
				{card.link}
			</ArrowButtonLabel>
		</a>
	);
...
function frameChrome() {
	...
	return (
		<>
			<span
				aria-hidden="true"
				className="pointer-events-none absolute inset-0 z-20 text-verevon-j-text/12"
			>
				<svg
			...
			{positions.map((position) => (
				<span
					aria-hidden="true"
					className={[
						"pointer-events-none absolute z-30 bg-verevon-j-text/16",
			...
			{corners.map((corner) => (
				<span
					aria-hidden="true"
					className={`pointer-events-none absolute z-40 size-[6px] rounded-full bg-verevon-j-text/24 ${corner}`}
					key={corner}
				/>
			))}
		</>
	...
	return (
		...
		>
			...
			>
				...
				<p className="mx-auto mt-4 max-w-[540px] font-protokoll text-[clamp(0.95rem,0.95vw,1.08rem)] font-light leading-[1.42] text-verevon-text-muted">
		...
		<div className="hidden max-[899px]:grid max-[899px]:gap-10 max-[899px]:pt-10">
			<div className="text-center">
				...
				<p className="mx-auto mt-5 max-w-[34rem] font-protokoll text-[1rem] font-light leading-[1.48] text-verevon-text-muted">
					Verevon samler kilder, lager forslag og viser hva som må godkjennes
			...
			<div className="mx-auto grid w-full max-w-[760px] gap-8">
				...
				<VerevonComposerPreview prompt={promptText} />
			</div>
		...
		const context = gsap.context(() => {
			...
			const timeline = gsap.timeline({
				...
				scrollTrigger: {
					...
					refreshPriority: 3,
					id: "verevon-trust-horizontal-scroll",
				},
			});
		...
		>
			...
			>
				...
				>
					...
					<div
						className="absolute z-20 overflow-hidden border border-verevon-j-text/8 bg-[#f8f8f7] shadow-[0_18px_56px_rgba(23,23,23,0.065)] max-[899px]:relative max-[899px]:left-auto max-[899px]:top-auto max-[899px]:h-auto max-[899px]:w-full max-[899px]:aspect-[1.5]"
						data-trust-frame=""
					>
					...
					<div className="hidden max-[899px]:grid max-[899px]:gap-10">
						{copyPanels.map((panel, index) => (
							<div
								...
								key={panel.body}
							>
								<span className="mb-4 block font-arbeit text-[0.76rem] text-verevon-coral">
									{String(index + 1).padStart(2, "0")}
								</span>
								<p className="m-0 font-protokoll text-[1.08rem] font-light leading-[1.45] text-verevon-j-text/68">
									{panel.body}
								</p>
							</div>
						))}
