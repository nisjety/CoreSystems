 ...
 *
 * Perf-safe by design:
 *  - The VEREVON wordmark is plain text painted at first paint, so it (not a
...
export function Preloader() {
	...
	useEffect(() => {
		...
		let seen = false;
		try {
			seen = sessionStorage.getItem("verevon-intro-seen") === "1";
		} catch {
			seen = false;
		}
		...
		try {
			sessionStorage.setItem("verevon-intro-seen", "1");
		} catch {
			/* ignore */
		}
	...
	return (
		<div
			aria-hidden="true"
			className={["verevon-preloader", phase === "reveal" ? "is-revealing" : ""]
				.filter(Boolean)
				.join(" ")}
		>
			<div className="verevon-preloader__inner">
				<span className="verevon-preloader__word">VEREVON</span>
				<span className="verevon-preloader__bar">
					<span className="verevon-preloader__bar-fill" />
				</span>
				<span className="verevon-preloader__label">
					<span className="verevon-preloader__label-no">01</span> — Norsk
					AI-arbeidsbenk
				</span>
			</div>
		</div>
