...
export function FeatureCardsSectionV2() {
	...
	return (
		<section
			...
			className="relative isolate overflow-hidden border-b border-verevon-j-text/8 bg-background px-[var(--verevon-edge)] py-[var(--verevon-section-vpad)] text-verevon-j-text max-[760px]:px-[var(--verevon-page-pad)]"
			id="produkt"
		>
			<div
				aria-hidden="true"
			...
			<div className="relative z-[1] mx-auto max-w-[1680px]">
				<div className="mx-auto flex max-w-[660px] flex-col items-center gap-7 text-center">
					<p className="verevon-eyebrow">Forankret handling</p>

					<h2
						className="fade-out-top m-0 font-arbeit text-[clamp(2.6rem,5vw,6.4rem)] font-light leading-[0.94] tracking-[-0.07em] text-verevon-j-text text-balance"
						data-fade-out-top
					>
						Tenking, satt i arbeid
					</h2>

					<p className="verevon-body-lg max-w-[540px] text-pretty">
				...
				<div
					className="verevon-fly-stage relative mt-[clamp(52px,6.5vw,96px)]"
				...
				>
					...
					<div className="relative z-[1] grid gap-x-[clamp(16px,1.6vw,28px)] gap-y-[clamp(28px,3vw,40px)] sm:grid-cols-2 xl:grid-cols-4">
						{cards.map((card, index) => (
							<article
								className="verevon-fly-card group flex min-w-0 flex-col gap-3"
								key={card.title}
								style={
									{
										"--fly-x": flyX[index],
							...
							>
								...
								<div className="relative aspect-[3/4] overflow-hidden rounded-[4px] border border-verevon-j-text/8 bg-white/55 shadow-[var(--verevon-shadow-sm)]">
									<Image
										alt=""
									...
									<h3 className="absolute bottom-4 left-4 right-4 m-0 font-arbeit text-[clamp(1.45rem,1.7vw,2rem)] font-light leading-[1.04] tracking-[-0.05em] text-verevon-j-text">
										{card.title}
									</h3>
								</div>

								<p className="m-0 font-protokoll text-[var(--text-body-sm)] font-light leading-[1.4] text-verevon-text-muted/90">
									{card.text}
								</p>
							</article>
						))}
					...
					{["left-0 top-0", "right-0 top-0", "left-0 bottom-0", "right-0 bottom-0"].map(
						(corner) => (
							<span
								aria-hidden="true"
								className={`pointer-events-none absolute z-[2] size-[6px] rounded-full bg-verevon-j-text/20 ${corner}`}
								key={corner}
							/>
						),
					)}
