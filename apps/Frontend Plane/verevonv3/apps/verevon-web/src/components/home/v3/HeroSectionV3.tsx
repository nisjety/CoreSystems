import { VerevonMarkOutline } from "../sections/VerevonMark";

/**
 * HeroSectionV3 — calm, ambient, fast.
 *
 ...
 *
 * Uses verevon-hero-approved-action.mp4 rather than the plain premiere-light
 * cut — same drone footage, but with a real "Trust Trail / Approved action"
 * card composited into the shot (source region, ZDR, audit trail). That
 * already proves the "signal to approved action" claim inside the footage
 * itself, so there is no separate DOM-built proof card layered on top of it
 * — a second card would either duplicate the message or collide with the
 * one baked into the video.
 */
		...
		>
			...
			>
				<video
					...
					muted
					playsInline
					poster="/verevon-vibe/verevon-hero-approved-action-poster.jpg"
					preload="metadata"
				>
					<source
						src="/verevon-vibe/verevon-hero-approved-action.mp4"
						type="video/mp4"
					/>
				</video>
			</div>
			...
			<div className="relative z-[1] mx-auto flex w-full max-w-[1680px] flex-col px-[var(--verevon-edge)] pb-[clamp(112px,15vh,178px)] pt-[clamp(150px,18vh,220px)] max-[760px]:px-[var(--verevon-page-pad)] max-[760px]:pb-[96px]">
				<h1 className="max-w-[13ch] font-arbeit text-[clamp(2.85rem,5.6vw,6.9rem)] font-light leading-[0.96] tracking-[-0.045em] text-verevon-c-white text-balance">
					Fra signal til handling.
				</h1>

				<p className="mt-6 max-w-[34rem] font-protokoll text-[clamp(0.9rem,0.9vw,1.03rem)] font-light leading-[1.42] text-[color-mix(in_srgb,var(--verevon-c-white)_72%,transparent)] text-pretty">
					Kilder, svar og godkjenning i én arbeidsflyt.
				</p>

				<div className="mt-7">
					<a
						className="group inline-flex w-fit items-center gap-3 font-protokoll text-[clamp(0.94rem,0.95vw,1.08rem)] font-light leading-none text-[color-mix(in_srgb,var(--verevon-c-white)_72%,transparent)] transition-colors duration-300 hover:text-verevon-c-white"
						href="#produkt"
					>
