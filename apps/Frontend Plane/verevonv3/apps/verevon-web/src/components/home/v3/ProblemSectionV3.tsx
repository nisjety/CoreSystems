...
// statement's whitespace, curated against Verevon's actual visual language
// (see public/verevon-vibe/*: soft glossy renders, macro folds, warm glow on
...
export function ProblemSectionV3() {
	return (
		<section
			className="relative isolate overflow-hidden bg-background px-[var(--verevon-edge)] py-[var(--verevon-section-vpad)] text-verevon-j-text max-[760px]:px-[var(--verevon-page-pad)]"
			id="problemet"
		>
			...
			<div className="relative z-10">
				...
				<div className="group/galaxy relative">
					...
					<div className="relative z-[1]">
						...
						<h2 className="m-0 max-w-[14ch] font-arbeit text-[clamp(2.6rem,5.5vw,6.5rem)] font-light leading-[0.98] tracking-[-0.06em] text-verevon-j-text text-balance">
							<Reveal>
						...
						<Reveal delay={180}>
							<p className="verevon-body-lg mt-8 max-w-[640px] text-pretty">
				...
				<div className="mt-[clamp(56px,7vw,104px)] grid grid-cols-3 gap-x-10 gap-y-10 max-[760px]:grid-cols-1">
					{cards.map((card) => (
						<a
							className={cn(
								"group block border-t border-verevon-j-text/12 pt-7 transition-all duration-300",
								"hover:-translate-y-0.5 hover:border-verevon-j-text/30",
							)}
							href={card.href}
							key={card.kicker}
						>
							<span className="font-protokoll text-[0.72rem] uppercase tracking-[0.14em] text-verevon-j-text/40">
								{card.kicker}
							</span>

							<h3 className="mt-4 font-arbeit text-[clamp(1.5rem,2vw,2.1rem)] font-light leading-[1.05] tracking-[-0.03em] text-verevon-j-text">
								{card.title}
							</h3>

							<p className="verevon-body mt-3 max-w-[34ch] text-pretty">
								{card.body}
							</p>
