...
export function SensesSectionV2() {
	return (
		<section
			className="grid min-h-[100svh] grid-cols-[minmax(320px,0.78fr)_minmax(0,0.92fr)] items-center gap-[clamp(48px,7vw,140px)] border-b border-verevon-j-text/8 bg-background px-[var(--verevon-edge)] py-[var(--verevon-section-vpad)] text-verevon-j-text max-[1100px]:grid-cols-1 max-[760px]:px-[var(--verevon-page-pad)]"
			id="kunnskap"
		>
			<Reveal className="w-full max-[1100px]:max-w-[720px]">
				<VisualPanel
					className="w-full"
					label="Verevon kildegraf og kunnskapsspor"
					variant="open"
				/>
			</Reveal>
			...
			<Reveal className="max-w-[760px] justify-self-end max-[1100px]:justify-self-start">
				<span
					aria-hidden="true"
					className="mb-8 block h-px w-[clamp(88px,9vw,168px)] bg-[color-mix(in_srgb,var(--verevon-a-earth)_28%,var(--verevon-border-strong))]"
				/>

				<h2
					className="fade-out-top m-0 max-w-[820px] font-arbeit text-[clamp(2.6rem,5vw,6rem)] font-light leading-[0.94] tracking-[-0.06em] text-verevon-j-text text-balance"
					data-fade-out-top
				>
				...
				<div
					...
					data-fade-out-top
				>
					<p className="verevon-body-lg m-0">
						Verevon kan koble til nettsider, dokumenter,
