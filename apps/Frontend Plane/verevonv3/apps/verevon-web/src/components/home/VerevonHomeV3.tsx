...
import { ApprovalSectionV3 } from "./v3/ApprovalSectionV3";
import { HeroSectionV3 } from "./v3/HeroSectionV3";
import { LayerSectionV3 } from "./v3/LayerSectionV3";
import { ProblemSectionV3 } from "./v3/ProblemSectionV3";
import { ProductLoopSectionV3 } from "./v3/ProductLoopSectionV3";
import { SensesSectionV3 } from "./v3/SensesSectionV3";

/**
 * VerevonHomeV3 — the argument-driven homepage variant.
 *
 * Same wedge as V1/V2, restructured so each section makes a distinct move
 * instead of restating "governed AI workflow loop" five times: hero (promise)
 * -> brand loop (fits your stack) -> problem (why now) -> senses (beliefs) ->
 * product loop (the demo, once) -> approval (the wedge: a human says yes) ->
 * layers (how it hangs together) -> pre-footer (mirror close).
 *
 * Reuses (does not modify) Navbar, MenuModal, Footer, BrandLogosSection and
 * PreFooterStatementSection. Everything else lives under ./v3 as net-new or
 * reworked copies of the V1 sections — V1/V2 source files are untouched.
 */
export function VerevonHomeV3() {
