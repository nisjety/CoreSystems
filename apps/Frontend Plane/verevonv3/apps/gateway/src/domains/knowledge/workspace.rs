...
//! Verevon v3 Knowledge surface renders: document-derived sources with chunk
...
//!
//! This is the Rust port of verevonv2's `loadKnowledgeWorkspace`
...
mod tests {
    ...
    fn min_center_distance(nodes: &[Value]) -> f64 {
        ...
        for i in 0..nodes.len() {
            for j in (i + 1)..nodes.len() {
                let (xi, yi) = (
                    nodes[i]["x"].as_f64().unwrap(),
                    nodes[i]["y"].as_f64().unwrap(),
                );
                let (xj, yj) = (
                    nodes[j]["x"].as_f64().unwrap(),
                    nodes[j]["y"].as_f64().unwrap(),
                );
    ...
    fn build_graph_gives_each_group_a_distinct_ring() {
        ...
        assert_eq!(
            radii.len(),
            4,
            "expected 4 distinct ring radii, got {radii:?}"
        );
    }
