...
async fn main() -> Result<()> {
    ...
    info!(%addr, "verevon-gateway-rs listening");
...
async fn health() -> Json<Value> {
    Json(json!({ "status": "ok", "service": "verevon-gateway-rs" }))
}
...
mod tests {
    ...
    fn nginx_access_log_uses_a_query_redacting_callback_target() {
        ...
        assert!(nginx.contains("map $uri $verevon_log_uri"));
        ...
        assert!(nginx.contains("log_format verevon_safe"));
        assert!(nginx.contains("access_log /var/log/nginx/access.log verevon_safe;"));

        let safe_format = nginx
            .split("log_format verevon_safe")
            .nth(1)
            .and_then(|suffix| suffix.split(';').next())
            .expect("verevon_safe log format");
        assert!(safe_format.contains("$verevon_log_uri"));
    ...
    fn client_actor_is_ignored_unless_dev_actor_mode_is_explicit() {
        ...
        assert_eq!(actor.user_id, "verevon-v3-local-user");
    ...
    /// authenticated user forges `x-verevon-org-id` for a victim org; the gateway
    ...
    async fn forged_org_header_never_reaches_upstream() {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    ...
                    .header("x-verevon-org-id", "org-victim")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        for req in &received {
            ...
            assert_ne!(
                forwarded,
                Some("org-victim"),
                "a client-forged x-verevon-org-id must never be forwarded as x-org-id"
            );
        }
    }
