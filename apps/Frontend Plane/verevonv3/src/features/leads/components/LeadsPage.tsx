...
export function LeadsPage() {
  ...
  return (
    <section class="verevon-leads" aria-label={t.title}>
      <header class="verevon-leads__head">
        <h1>{t.title}</h1>
        <p class="verevon-leads__sub">{t.subtitle}</p>
      </header>

      <form class="verevon-leads__filters" onSubmit={runSearch}>
        <label>
      ...
      <p class="verevon-leads__hint">{t.employeeBandHint}</p>

      <Show when={searchError()}>
        <p class="verevon-leads__error" role="alert">{searchError()}</p>
      </Show>

      <Show when={searched() && !searching() && !searchError()}>
        <Show
          when={results().length > 0}
          fallback={<p class="verevon-leads__muted">{t.noResults}</p>}
        >
          <form class="verevon-leads__save" onSubmit={saveList}>
            ...
            <Show when={saveMessage()}><span class="verevon-leads__note" role="status">{saveMessage()}</span></Show>
          </form>

          <table class="verevon-leads__table">
            <thead>
              <tr>
                <th aria-label="Velg" />
      ...
      <section class="verevon-leads__lists" aria-label={t.savedLists}>
        <h2>{t.savedLists}</h2>
        <Show when={!lists.loading} fallback={<p class="verevon-leads__muted">{t.loading}</p>}>
          <Show when={(lists() ?? []).length > 0} fallback={<p class="verevon-leads__muted">{t.noSavedLists}</p>}>
            <ul class="verevon-leads__listrows">
              <For each={lists()}>
                {(list) => (
                  <li class="verevon-leads__listrow">
                    <button type="button" class="verevon-leads__listname" onClick={() => open(list)}>
                      {list.name} <span class="verevon-leads__count">{list.company_count}</span>
                    </button>
                    <div class="verevon-leads__listactions">
                      {/* Metered: the gateway gates this on the org's billing entitlement. */}
                      <a class="verevon-leads__btn" href={leadExportHref(list.id)} download={`${list.name}.csv`}>{t.exportCsv}</a>
                      <button type="button" class="verevon-leads__btn verevon-leads__btn--danger" onClick={() => remove(list)}>{t.delete}</button>
                    </div>
                  </li>
                )}
              </For>
        ...
        <Show when={openList()}>
          {(list) => (
            <div class="verevon-leads__viewer">
