...
function SafeEmailFrame(props: { html: string }) {
  ...
  return (
    <iframe
      ref={frame}
      class="verevon-inbox-email-frame"
...
function PlainTextBody(props: { text: string }) {
  ...
  return (
    <Show when={props.text.trim()} fallback={<span class="verevon-inbox-article__empty">{i18n.tr('Ingen meldingstekst.', 'No message body.')}</span>}>
      <div class="verevon-inbox-email-plain">
        <For each={segments()}>
          {(segment) =>
            segment.url ? (
