...
import {
  ...
  workflowToolIconMap,
} from '@/features/agents/lib/verevon-workflow-builder-data'
...
export function WorkflowToolsPanel(props: {
  ...
  return (
    <aside
      ...
      class={cn(
        'verevon-sidebar-type flex h-full min-w-0 flex-col bg-[#F7F7F8] font-sans text-[#25272D] dark:bg-[#101114] dark:text-white',
        props.class,
      )}
    >
      <div class="flex h-11 shrink-0 items-center justify-between gap-3 px-1">
        <div class="flex items-center gap-2.5">
          ...
          <h2 class="verevon-sidebar-group-title">{i18n.tr('Verktøy', 'Tools')}</h2>
        </div>
        <div class="flex items-center gap-1">
          <VerevonIconButton type="button" size="xs" shape="rounded" aria-label={i18n.tr('Flere alternativer for arbeidsflytverktøy', 'More workflow tools options')}>
      ...
      <div class="px-1 pb-3">
        <label class="relative block">
          ...
          <input
            ...
            class="verevon-field-compact h-9 pl-9 pr-3 placeholder:text-[#B2B6BE] dark:placeholder:text-[#777E8B]"
          />
        </label>
      </div>

      <div class="verevon-sidebar-row-strong grid grid-cols-4 border-b border-[#E1E4E8] px-1 dark:border-[#292C33]">
...
function WorkflowToolCard(props: {
  ...
  return (
    ...
    >
      ...
      <span class="verevon-sidebar-row-strong mt-2.5 max-w-full">{props.label}</span>
    </button>
  )
}
