...
export function formatPlanLabel(plan?: string | null): string {
  ...
  const labels: Record<string, string> = {
    ...
    free: 'Free',
    hobby: 'Hobby',
    // Verevon brand name for the `pro` billing tier (billing-core: "Verevon Expert").
    pro: 'Expert',
    standard: 'Standard',
    trial: 'Trial',
  }
...
export function getNavbarLabels(activeRoute: VerevonRoute, locale: Locale = 'no') {
  ...
  switch (activeRoute) {
    ...
    case '/tickets':
    case '/support':
      return labels('Support', 'Support', 'Samtaler og saker', 'Conversations and tickets')
    case '/social':
...
export function routeFromPath(pathname: string): VerevonRoute {
  ...
  // Keep existing inbound links working, but give them the single Support
  // navigation context rather than exposing two competing products.
  if (path.startsWith('/inbox')) return '/support'
