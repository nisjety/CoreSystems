...
/**
 * Reserve a popup while the click event still has browser user activation.
 *
 * The connect-session request is asynchronous. Opening only after it resolves
 * is commonly blocked by Safari and privacy-hardened browsers, which leaves a
 * pending session that the operator cannot approve. Call this synchronously in
 * the click handler, then pass the returned window to runDirectOauthWindow.
 */
export function reserveDirectOauthWindow(): Window | null {
  return openProviderAuthWindow('about:blank')
}
...
export async function runDirectOauthWindow(input: {
  ...
  sessionToken?: string
}, reservedWindow?: Window | null): Promise<void> {
  const expectedOrigin = new URL(input.connectUrl).origin
  const authWindow = reservedWindow ?? openProviderAuthWindow(input.connectUrl)
  if (!authWindow) {
