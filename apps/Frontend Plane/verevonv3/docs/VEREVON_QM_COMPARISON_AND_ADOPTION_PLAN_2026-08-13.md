...
| S1.5 through S1.7 | `[~]` 2026-08-25: S1.5 has a fail-closed Data mapping/schema, boundary Control decision verification on both the HTTP and gRPC retrieval paths, and a real owner-plane binding writer/revoker (`POST /v1/internal/space-retrieval-bindings[/delete]`, advisory-locked, idempotency-receipted, audited). Open: the mapping carries no revision column, so freshness rests on exact `resource_authorization_ref` equality and the decision's `authority_revision` is validated as positive but never compared against stored state; scoped retrieval stays dense-only until the sparse/graph/wiki/visual/keyword arms carry the same owner-resource predicate; deployment provisioning, mapping backfill, Ingestion job families, and the remaining owner slices are open. |
...
- [ ] Version the mapping itself. `space_retrieval_bindings` has no revision column, so a superseded mapping is only distinguishable through revoked rows plus the `admin_audit_log` receipt, and `space_scope.rs` validates the decision's `authority_revision` as positive without ever comparing it to stored state. Add a monotonic binding revision, record the authority/membership/privacy revisions the mapping was provisioned under, and reject a decision whose revisions are older than the active row so a stale-but-unexpired decision cannot resolve a re-pointed Space.
