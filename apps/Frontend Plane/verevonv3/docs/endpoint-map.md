# verevonv3 Endpoint Map

Complete catalog of backend endpoints across the five reachable planes, annotated for verevonv3 consumption.
Generated 2026-06-10 from `docs/core-research/*` (per plane) plus route registrations verified in source
(verevon-gateway-rs `src/`, model-gateway `src/http_routes.rs`, quarry-edge `src/routes.rs`, integration-corev2
`internal/api/server.go`, each Control Plane core's `internal/http/server.go`, etc.).

**Architecture rule (fixed):** verevonv3 (SolidJS SPA, no BFF) talks ONLY to `verevon-gateway-rs` (Application
...
| NATS | `aqencia.controlplane.*`, `verevon.audit.v1.control.*` | Event fan-out (audit only with org_id) | NATS creds | NATS | none |
...
| NATS | sub `verevon.audit.v1.>`, `verevon.usage.v1.>` | Best-effort ingest (no retry/NAK) | NATS creds | NATS | none |
...
verified Verevon session. The gateway derives the tenant from canonical session
...
| GET | `/v1/wiki/pages` | Paginated page list (built for the verevon sidebar) | `wiki.read` bearer | HTTP | core |
...
| POST | `/v1/audio` | Audio evidence ingestion; not the primary Verevon dictation path | Bearer JWT | HTTP | **later** |
...
| support-worker | NATS→Temporal bridge (`verevon.support.>`), no API | none |
...
### 4.1 verevon-gateway-rs — HTTP :3185 (THE v3 backend)

Today: 20 routes (health + onboarding domain). No inbound token validation — actor from trusted proxy headers (`x-session-user-id/email/name`), dev headers behind `ALLOW_DEV_ACTOR_HEADERS`, silent fallback actor `verevon-v3-local-user`. Outbound: stamps `x-internal-api-key` + identity headers on every upstream (12s reqwest timeout). CORS allowlist defaults to Vite dev origins. Several handlers mask upstream failures as 200 + `success:false`.
...
JWT Bearer via JWKS from auth-core on all non-health routes; rate-limited; dev bypass `MODEL_GATEWAY_AUTH_DEV_BYPASS=1`. verevon-gateway-rs mints the user's `model-plane` audience token and forwards it.
...
Endpoints listed in call order where order matters. Everything routes through verevon-gateway-rs.
