...
Verevon v3 is a client-side SolidJS app mounted from `src/index.tsx` (`render(() => <App />, root)`)
...
- The shell reads the live session via `getSession()` and fetches navbar state from the gateway
  ...
  180s hidden) and on tab focus. There is no mock operating model; `verevon-operating-model.ts`
...
- The same-origin BFF normalization layer is the Rust gateway at `apps/gateway`
  ...
  the verified session, not from a client-supplied `x-verevon-org-id` header (the earlier
