# Verevon v3 Auth Boundary

> Verified 2026-07-11 (source-only; SPA :5173 + verevon-gateway-rs :3185 containers down this pass).
...
Verevon v3 authenticates for real. There is no local mock and no direct-to-onboarding shortcut.
...
- `middleware.rs::strip_inbound_identity_headers` runs globally before routing and removes
  every `STRIPPED_HEADERS` entry — including `x-org-id` and `x-verevon-org-id` — so a
  browser-forged identity/tenant header never reaches a handler. Matching is case-insensitive,
  so `X-Verevon-Org-Id` is dropped too.
- Org scoping is derived server-side from the validated session (`active_org_id`, with a
  ...
  cross-tenant `x-verevon-org-id` IDOR found in the AI-First audit; it is covered by tests in
  `middleware.rs` and `main.rs` ("a client-forged x-verevon-org-id must never be forwarded as
  x-org-id").
