# Verevon v3 Onboarding Gateway Integration
...
Onboarding is the strongest live integration in Verevon v3.
...
2. optional absolute `VITE_VEREVON_GATEWAY_URL` override

In development, `vite.config.ts` proxies `/api` and `/health` to `GATEWAY_PROXY_TARGET`, `VITE_VEREVON_GATEWAY_URL`, or `http://127.0.0.1:3185`. That target is the Verevon v3 Rust gateway under `apps/gateway`.
...
- Control Plane, Ingestion Plane, Data Plane, Model Plane, and Application Plane are reached indirectly through the Verevon v3 Rust gateway.
- Onboarding state is persisted both locally and remotely.
- Local persistence uses `localStorage` key `verevonv3.onboarding.state.v1`.
- Actor fallback uses `localStorage` key `verevonv3.onboarding.actor`.
...
- The default actor is `verevon-v3-local-user` with `local@verevon.dev`.
...
The historical `x-verevon-org-id` cross-tenant IDOR remains fixed. `x-verevon-org-id` (case-insensitively, so `X-Verevon-Org-Id` too) is in the gateway's `STRIPPED_HEADERS` and dropped globally at ingress by `strip_inbound_identity_headers` (`apps/gateway/src/middleware.rs`). The org is derived server-side from the validated Better Auth session (`active_org_id` ← `activeOrganizationId`), never from a client header. Regression tests: `middleware.rs` (strip assertions) and `src/main.rs` (a forged `x-verevon-org-id` is never forwarded as `x-org-id`).

## Notes

The dependency on the Rust gateway is important because Application Plane research still contains older `verevon-gateway-rs` notes. For Verevon v3, the current source path and proxy setup make this a Frontend Plane gateway unless a deployment decision says otherwise.
