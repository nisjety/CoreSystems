...
- AG-UI/TanStack tool specs with JSON-schema parameters for the Verevon Gateway request body.
...
This is the Verevon v3 AI-first invariant for the chat surface: actions that can be presented in the UI
...
- for live actions it `POST`s to `/api/v1/actions/execute` with body `{ actionId, input }` and header
  `x-verevon-org-id: <actor.orgId>`, returning the server-provided `ActionExecution`
...
- Cross-tenant scoping (IDOR re-check, verified 2026-07-11): the dispatchers derive org from
  ...
  (active org / session-context `orgId`). The client-sent `x-verevon-org-id` header is **not** trusted
...
- Model-tier routing is server-owned. Chat exposes Budget/Balance/Genius intent
  modes, while Model Plane resolves the concrete provider/model from its
  authoritative complexity and budget policy. The obsolete local
  `cost-policy.ts` selector was removed rather than allowing a second,
  client-controlled routing authority.
...
`apps/gateway/src/domains/actions/`. Remaining follow-up is filling in
dispatchers for any descriptors still returning `501`; model routing stays with
Model Plane rather than being recreated in the browser.
