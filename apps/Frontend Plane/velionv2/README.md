...
Verevon is the frontend workspace for Verevon, an AI-first customer experience system where the AI worker, also named Verevon, is the main operating layer for support, sales assistance, knowledge, and workflow execution.

The product goal is not "a helpdesk with AI features." Verevon should feel like an AI teammate that can understand a company, propose work, execute approved actions, and keep humans in control. Every task Verevon can do autonomously must also be possible manually in the UI.

## Product Vision

Verevon competes with Intercom, Chatbase, Gorgias, Zendesk, and Norwegian ecommerce AI support products such as Mimir. The differentiator is an AI-native operating model:

- A user can ask Verevon to create and deploy a chatbot.
- A user can ask Verevon to answer or draft email, chat, and social support replies.
- A user can ask Verevon to connect sources, crawl a website, inspect the knowledge graph, and explain what it learned.
- A user can ask Verevon to create support policies, macros, workflows, handoff rules, and routing.
- Verevon can act across connected systems only within explicit permission, approval, audit, and rollback boundaries.
- Human users can inspect sources, override decisions, approve risky actions, and manually perform the same work.

## Core Surfaces

- `Onboarding`: identify the organization, crawl the website, connect sources, preview the graph, prove that Verevon understands the business, and recommend a plan.
- `Inbox`: unified conversations across support channels, with Verevon drafting, summarizing, tagging, routing, and escalating.
...
- `Chat`: talk directly to Verevon to plan and execute work.
...
Verevon should run every meaningful action through the same lifecycle:
...
Manual parity is required. If Verevon can deploy a widget, update a macro, answer an email, create a workflow, disconnect a source, or change a plan, the UI must expose the same action directly.

## Architecture

Verevon v2 is a clean Next.js 16 App Router rebuild. The old app is product and roadmap input only; v2 intentionally does not import v1 code or architecture.

```mermaid
flowchart LR
  User["User or customer"] --> Frontend["Verevon v2 frontend"]
  ...
  BFF --> Model["Model Plane: Verevon AI, recommendations, planning"]
...
- `src/features/chat-v2`: Verevon chat surface.
...
- Verevon is the AI worker at the center of the product, not a side panel.
- The first run experience must prove value before the paywall by showing what Verevon learned and what it can improve.
- Users should never feel trapped by automation. There must always be review, approval, handoff, rollback, and manual control.
- Source trust matters. Every answer and recommendation should show what data Verevon used and what is missing.
- Integrations must be connected to outcomes, not treated as setup chores.
- The UI should be dense, calm, and operational, with moments of proof where Verevon explains its understanding.
