...
SCRIPT="$TEST_DIR/../build-velion-services.sh"
TMP_DIR="$(mktemp -d)"
...
new_fixture() {
  ...
  for compose_file in \
    ...
    "apps/Frontend Plane/velionv3/docker-compose.production.yml"; do
...
test_from_selects_remaining_stacks() {
  ...
  assert_contains "--from continues through Frontend" "$output" "Building and starting Frontend Plane Velion v3"
}
...
test_production_overrides_are_rendered() {
  ...
  assert_contains "production validation layers the Frontend override" "$calls" \
    "-f apps/Frontend Plane/velionv3/docker-compose.yml -f apps/Frontend Plane/velionv3/docker-compose.production.yml"
}
