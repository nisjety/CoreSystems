...
/**
 * Local-stack auth setup: signs the pre-provisioned `local@velion.dev`
 ...
 * targets — start authenticated. Mirrors auth.setup.ts's e2e@velion.dev flow.
 *
 * Unlike e2e@velion.dev, this account is seeded with its own "Velion" org by
 * build-velion-services.sh's seed_dev_account step, so it should already be
 ...
 *   bash "apps/Frontend Plane/velionv3/build-velion-services.sh"
 ...
 */
const EMAIL = process.env.LOCAL_E2E_EMAIL || 'local@velion.dev'
const PASSWORD = process.env.LOCAL_E2E_PASSWORD || 'Velion-Admin-2026!'
const STORAGE_STATE = 'tests/e2e/.auth/local-state.json'

setup('authenticate local@velion.dev', async ({ page, baseURL }) => {
  ...
  // 3. Defensive org bootstrap — this account should already own "Velion",
