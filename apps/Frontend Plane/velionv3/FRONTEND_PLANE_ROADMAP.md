# Frontend Plane (velionv3) — Roadmap
...
Derived from `docs/core-research/plane-audit-2026-07-11.md`. Read `FRONTEND_PLANE_STATUS.md` first. velionv3 SPA changes hot-reload (Vite, bind-mounted source) so SPA fixes go live without a Docker rebuild; gateway (Rust) fixes need a rebuild (currently blocked by the Docker maintenance gate).

## Phase A — Security (the new IDORs)

1. **Fix the `onboarding/graph-preview` cross-tenant IDOR (HIGH)** — `apps/gateway/src/onboarding/lookup/graph.rs` must derive org from the verified session (`authorized_org_id(&user)`), exactly like its sibling `translate_recommendation`, and ignore the client `org_id` query param. Today any authenticated user reads any org's knowledge graph via `?orgId=<victim>`. Add a regression test mirroring the `x-velion-org-id` strip tests.
...
3. Remove `x-velion-org-id` from the CORS allow-headers list (defense-in-depth; it's stripped at ingress anyway, but advertising it invites a future handler to read it).
...
3. For connector-style remote servers like Visma Net (OAuth Streamable-HTTP), the real blocker is Model Plane lacking an OAuth remote-MCP client — see Model Plane roadmap. The velionv3 form is ready once that backend support exists.
