...
/** Small uppercase kicker above a heading. Token-backed (.velion-eyebrow). */
export function Eyebrow({ children, className = "", marker = false }: EyebrowProps) {
	return (
		<p
			className={["velion-eyebrow inline-flex items-center gap-3", className]
				.filter(Boolean)
				.join(" ")}
		>
			{marker ? (
				<span
					aria-hidden="true"
					className="h-px w-8 bg-velion-coral/70"
				/>
			) : null}
			{children}
		</p>
	...
	return (
		...
		>
			...
			{lede ? (
				<p
					className={[
						"velion-body-lg mt-7 text-pretty",
