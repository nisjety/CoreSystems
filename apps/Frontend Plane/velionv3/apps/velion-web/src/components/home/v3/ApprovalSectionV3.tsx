...
const pillars: TrustPillar[] = [
	...
	},
	{
		...
		body: "Velion håndhever Zero Data Retention i modell-laget, slik at ZDR-forespørsler ikke havner i varig prompt-cache.",
		level: "live",
	},
	{
	...
	},
	{
		...
		body: "Norske virksomheter kobles mot Enhetsregisteret, slik at Velion jobber med ekte selskapsdata.",
		level: "live",
	},
];
...
function ApprovalField({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex flex-col gap-1.5">
			<dt className="font-protokoll text-[0.7rem] font-medium uppercase tracking-[0.1em] text-velion-j-text/42">
				{label}
			</dt>
			<dd className="m-0 font-protokoll text-[0.94rem] font-light leading-[1.4] text-velion-j-text">
				{value}
			</dd>
		</div>
	);
 ...
 * Velion's approval queue. Purely illustrative: nothing here is wired, so
 * every control is inert (matching the VelionComposerPreview convention of
...
function ApprovalQueueCard() {
	return (
		<div
			...
			className="relative w-full max-w-[480px] border border-velion-j-text/10 bg-white/52 p-[clamp(22px,2.6vw,32px)] shadow-[0_28px_72px_rgba(23,23,23,0.08)] backdrop-blur-[14px]"
		>
			<div className="flex items-center justify-between gap-3">
				<span className="font-protokoll text-[0.68rem] uppercase tracking-[0.16em] text-velion-j-text/42">
					Venter på godkjenning
				</span>
				<span
					aria-hidden="true"
					className="size-1.5 shrink-0 rounded-full bg-velion-coral motion-safe:animate-pulse"
				/>
			</div>
			...
			<div className="mt-7 flex flex-wrap items-center gap-3">
				<button
					className="rounded-full bg-velion-coral px-5 py-2.5 font-protokoll text-[0.86rem] font-medium text-white"
					tabIndex={-1}
					type="button"
				>
					Godkjenn og send
				...
				<button
					className="rounded-full border border-velion-j-text/20 px-5 py-2.5 font-protokoll text-[0.86rem] font-medium text-velion-j-text"
					tabIndex={-1}
					type="button"
				>
					Avvis
			...
			<div className="mt-6 border-t border-velion-j-text/10 pt-5">
				<h4 className="font-protokoll text-[0.7rem] font-medium uppercase tracking-[0.1em] text-velion-j-text/42">
					Revisjonsspor
				</h4>
				...
				<ul className="mt-3 flex flex-col gap-2">
					{auditTrailEntries.map((entry) => (
						<li
							className="flex items-center gap-2 font-protokoll text-[0.86rem] font-light text-velion-j-text/72"
							key={entry}
						>
							<Check className="size-3.5 shrink-0 text-velion-coral" />
							{entry}
						</li>
					))}
				</ul>
			...
			<div className="relative z-[1] mx-auto max-w-[1680px]">
				<div className="grid items-start gap-x-[clamp(40px,5vw,96px)] gap-y-14 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
					<div className="max-w-[760px]">
						...
						<h2 className="m-0 mt-6 max-w-[820px] font-arbeit text-[clamp(3.35rem,6.4vw,8.6rem)] font-light leading-[0.9] tracking-[-0.078em] text-velion-j-text text-balance">
							<Reveal>
						...
						<Reveal delay={90}>
							<p className="mt-[clamp(28px,3vw,46px)] max-w-[560px] font-protokoll text-[clamp(1.02rem,1.05vw,1.22rem)] font-light leading-[1.5] text-velion-text-muted text-pretty">
								Velion kan foreslå arbeid på tvers av kundeservice, kunnskap og
					...
					<div className="relative mx-auto w-full max-w-[480px] xl:mx-0 xl:ml-auto">
						<span
							aria-hidden="true"
							className="absolute -inset-3 -z-10 rotate-[-1.4deg] border border-velion-j-text/8 bg-white/30 backdrop-blur-[10px] max-[760px]:hidden"
						/>
				...
				<Reveal delay={200}>
					<ul className="mt-[clamp(48px,6vw,88px)] flex max-w-[560px] flex-col gap-3">
						{noListItems.map((item) => (
							<li className="velion-body-lg text-pretty" key={item}>
								<span className="text-velion-coral">Ingen</span>
