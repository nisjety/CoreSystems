...
const workflowStages: WorkflowStage[] = [
	{
		...
		image: "/velion-product-shots/dashboard-live-overview.png",
		mediaAlt:
			"Velion-oversikten med teamets daglige arbeid og innganger til samtaler, kilder og handlinger.",
	},
	{
		id: "draft",
		label: "Utkast",
		title: "Gjør grunnlaget om til et forslag.",
		body: "Velion forbereder et utkast med det den har funnet, slik at teamet kan se både svaret og hvorfor det ble foreslått.",
		detail: "Et utkast er et forslag — ikke en handling som skjer av seg selv.",
		meta: "Kilder vedlagt utkastet",
		image: "/velion-product-shots/chat-draft-sources-focus.jpg",
		mediaAlt:
			"Velion-chat med et svarutkast og synlige retningslinjekilder som ligger til grunn.",
	},
	{
		id: "approval",
		label: "Godkjenning",
		...
		image: "/velion-product-shots/chat-draft-answer-expanded.png",
		mediaAlt:
			"Velion-chat med et utkast til svar og en utvidet produktflate for vurdering.",
	},
	{
		id: "revision",
		label: "Revider",
		...
		mediaAlt:
			"Velion-chat med synlige agentsteg som viser hvordan arbeidet er bygget opp.",
	},
];
...
export function WorkflowExplorer() {
	...
	return (
		<section
			aria-label="Arbeidsløkken i Velion"
		...
		>
			<div className="mx-auto max-w-[1680px]">
				<div
					aria-label="Steg i Velions arbeidsflyt"
					className="grid gap-8 border-y border-velion-j-text/10 py-5 lg:grid-cols-4 lg:gap-0"
					role="tablist"
				>
					{workflowStages.map((stage, index) => {
						...
						return (
							<button
								...
								className={[
									"group grid grid-cols-[auto_1fr] items-start gap-3 rounded-sm text-left transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-velion-j-text lg:border-l lg:px-7 lg:first:border-l-0 lg:first:pl-0 lg:last:pr-0",
									isActive
										? "text-velion-j-text"
										: "text-velion-j-text/72 hover:text-velion-j-text",
								].join(" ")}
								id={tabId}
								key={stage.id}
							...
							>
								<span
									...
									isActive
										...
										: "border-velion-j-text/28 bg-white text-velion-j-text/68 group-hover:border-velion-j-text/44",
								].join(" ")}
								>
								...
								<span>
									...
									<span
										className={[
											...
											isActive
												? "text-velion-j-text/68"
												: "text-velion-j-text/62 group-hover:text-velion-j-text/72",
										].join(" ")}
									>
										{stage.detail}
									</span>
				...
				>
					<div className="max-w-[570px]">
						<div className="flex items-center gap-3 font-protokoll text-[0.72rem] font-medium uppercase tracking-[0.18em] text-velion-coral">
							<ActiveIcon aria-hidden="true" className="size-4" strokeWidth={1.6} />
							{String(activeStageIndex + 1).padStart(2, "0")} / {activeStage.label}
						</div>
						<h2 className="velion-h2 mt-5 max-w-[12ch] text-balance">
							{activeStage.title}
						</h2>
						<p className="velion-body-lg mt-7 max-w-[49ch] text-pretty">
							{activeStage.body}
						</p>
						<p className="mt-5 max-w-[49ch] font-protokoll text-[var(--text-body)] font-light leading-[1.55] text-velion-j-text/62 text-pretty">
							{activeStage.detail}
						</p>
						<div className="mt-9 inline-flex items-center gap-3 border-y border-velion-j-text/12 py-3 font-protokoll text-[0.82rem] font-light text-velion-j-text/62">
							<Check aria-hidden="true" className="size-4 text-emerald-700" strokeWidth={1.8} />
							{activeStage.meta}
						</div>
					</div>

					<figure className="relative aspect-[1.48] overflow-hidden rounded-[26px] border border-velion-j-text/10 bg-velion-surface-soft shadow-[0_28px_84px_rgba(23,23,23,0.10)]">
						<Image
						...
						<div className="absolute left-4 top-4 flex items-center gap-2.5 rounded-full border border-white/84 bg-white/90 px-3 py-2 font-protokoll text-[0.72rem] text-velion-j-text/66 shadow-[0_8px_20px_rgba(23,23,23,0.08)] backdrop-blur-md sm:left-6 sm:top-6">
							<span className="size-1.5 rounded-full bg-velion-coral" />
							Arbeidsflate i Velion
						</div>
						<div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-4 rounded-[16px] border border-white/80 bg-white/92 px-4 py-3 font-protokoll text-[0.74rem] font-light text-velion-j-text/62 shadow-[0_14px_34px_rgba(23,23,23,0.10)] backdrop-blur-md sm:bottom-6 sm:left-6 sm:right-6 sm:px-5">
							<span className="truncate">{activeStage.meta}</span>
							<ArrowRight aria-hidden="true" className="size-4 shrink-0 text-velion-j-text/62" strokeWidth={1.5} />
						</div>
					</figure>
				</div>
			</div>
