...
export function OrgDeletionDangerZone() {
  ...
  return (
    <Show when={isOrgOwner(session.activeOrg?.role)}>
      <div id="org-danger-zone" class="velion-privacy-danger">
        ...
        <Show when={showConfirm()}>
          <form class="velion-privacy-erase-form" onSubmit={runDelete}>
            <label>
            ...
            <div class="velion-privacy-erase-actions">
              ...
              <SettingsButton type="submit" settingsSize="sm" danger disabled={!canDelete()}>
                <Show when={deleting()} fallback={<>Delete organization</>}>
                  <Loader2 size={14} class="velion-trust-spin" aria-hidden="true" /> Scheduling…
                </Show>
              </SettingsButton>
            </div>
          </form>
