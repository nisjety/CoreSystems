    ...
    <div class="velion-insights-page">
      <section class="velion-insights-hero">
        <div>
          ...
          <h1>{i18n.tr('Velion målelag', 'Velion measurement layer')}</h1>
          <p>{config().description}</p>
        </div>
        <A href="/settings/integrations">{i18n.tr('Administrer tilkoblinger', 'Manage connectors')}</A>
      </section>

      <nav class="velion-insights-tabs" aria-label={i18n.tr('Innsikt-seksjoner', 'Insights sections')}>
        <For each={sections()}>
          {(section) => (
            <A
...
function LoadingState() {
  const i18n = useI18n()
  return (
    <section class="velion-insights-loading" aria-label={i18n.tr('Laster Innsikt', 'Loading Insights')}>
      <Gauge size={18} />
      <div>
...
// The pilot's "three honest numbers on one page" (see velion-feature-map.md
...
function HeadlineMetricsSection(props: { overview: InsightsWorkspace['overview'] }) {
  ...
  return (
    <section class="velion-insights-headline" aria-label={i18n.tr('Pilotens hovedtall', 'Pilot headline metrics')}>
      ...
      <ul class="velion-insights-headline-grid">
        <For each={metrics()}>
...
function HeadlineMetricCard(props: { metric: HeadlineMetric }) {
  const i18n = useI18n()
  return (
    <li class={cn('velion-insights-headline-card', `velion-insights-headline-card--${props.metric.state}`)}>
      <div class="velion-insights-headline-card__topline">
        <span>{props.metric.label}</span>
        <small>{headlineStateLabel(props.metric.state, i18n.tr)}</small>
      </div>
      <strong class="velion-insights-headline-card__value">{props.metric.value}</strong>
      <p class="velion-insights-headline-card__detail">{props.metric.detail}</p>
    </li>
  )
}
...
function ConnectorRegistrySection(props: { registry: InsightsWorkspace['insightConnectors'] }) {
  const i18n = useI18n()
  return (
    <section class="velion-insights-connectors" aria-label={i18n.tr('Innsikt-tilkoblingsregister', 'Insight connector registry')}>
      <div class="velion-insights-section-heading">
        <div>
          <span>
            <PlugZap size={14} />
...
function MetricsSection(props: { overview: InsightsWorkspace['overview'] }) {
  ...
  return (
    <section class="velion-insights-metrics" aria-label={i18n.tr('Målenøkkeltall', 'Measurement metrics')}>
      <div class="velion-insights-section-heading">
        <div>
          <span>
            <BarChart3 size={14} />
  ...
  return (
    <li class="velion-insights-metric-card">
      ...
      <Show when={props.scorecard.source}>
        <p class="velion-insights-metric-card__source">{i18n.tr(`Kilde: ${props.scorecard.source}`, `Source: ${props.scorecard.source}`)}</p>
      </Show>
    </li>
  )
}
...
function ExternalAnalyticsSection(props: { slots: ExternalAnalyticsSlot[] }) {
  const i18n = useI18n()
  return (
    <section class="velion-insights-external" aria-label={i18n.tr('Eksterne analysetilkoblinger', 'External analytics connectors')}>
      <div class="velion-insights-section-heading">
        <div>
          <span>
            <PlugZap size={14} />
