...
function railImage(container: HTMLElement): HTMLImageElement | null {
  return container.querySelector('.velion-chat-run-shot__frame img')
}
...
describe('ChatLiveRunPanel', () => {
  it('stays closed — and never subscribes — while the turn has no run id', () => {
    ...
    expect(container.querySelector('.velion-chat-run-panel')).toBeNull()
  ...
  it('opens and attaches to the run event stream once a run id arrives', () => {
    ...
    expect(container.querySelector('.velion-chat-run-panel')).not.toBeNull()
  ...
  it('collapses to its rail and restores the body', () => {
    ...
    expect(container.querySelector('.velion-chat-run-panel__body')).not.toBeNull()

    fireEvent.click(screen.getByLabelText('Skjul live-panelet'))

    expect(toggles).toEqual([true])
    expect(container.querySelector('.velion-chat-run-panel--collapsed')).not.toBeNull()
    expect(container.querySelector('.velion-chat-run-panel__body')).toBeNull()

    fireEvent.click(screen.getByLabelText('Vis live-panelet'))

    expect(toggles).toEqual([true, false])
    expect(container.querySelector('.velion-chat-run-panel--collapsed')).toBeNull()
    expect(container.querySelector('.velion-chat-run-panel__body')).not.toBeNull()
  ...
  it('renders a step screenshot from its artifact reference and expands it on click', () => {
    ...
    fireEvent.click(container.querySelector('.velion-chat-run-shot') as HTMLElement)

    const expandedImage = container.querySelector('.velion-chat-run-shot-expanded img')
  ...
  it('re-subscribes to the new run and drops the previous run\'s steps', () => {
    ...
    expect(container.querySelectorAll('.velion-chat-run-shot')).toHaveLength(0)
