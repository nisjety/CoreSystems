...
describe('ArtifactsPanel', () => {
  ...
  it('renders a code artifact with a line-number gutter', () => {
    ...
    const gutters = container.querySelectorAll('.velion-chat-artifact-code__gutter')
    expect([...gutters].map((node) => node.textContent)).toEqual(['1', '2'])
    // The language recovered from the title drives the highlighter.
    expect(container.querySelector('.velion-code-keyword')?.textContent).toBe('def')
  })
  ...
  it('toggles between the HTML preview and its source', () => {
    ...
    expect(container.querySelector('.velion-chat-artifact-code')).toBeTruthy()
  ...
  it('switches the viewer when another artifact is selected from the list', () => {
    ...
    expect(container.querySelector('.velion-chat-artifact-document')).toBeTruthy()

    screen.getByText('query.sql').click()
    expect(container.querySelector('.velion-chat-artifact-code')).toBeTruthy()
    expect(container.querySelector('.velion-chat-artifact-document')).toBeNull()
  })
})
