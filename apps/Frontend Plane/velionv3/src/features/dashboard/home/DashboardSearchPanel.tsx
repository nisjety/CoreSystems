...
const searchSessionKey = 'velion.dashboard.search.session'
...
type SearchSnapshot = {
  ...
  query: string
  savedAt: number
  velionResults: NavbarSearchResult[]
...
export function SearchPanel(props: {
  ...
  let velionController: AbortController | undefined
  ...
  const [velionResults, setVelionResults] = createSignal<NavbarSearchResult[]>(restored?.velionResults ?? [])
  const [velionLoading, setVelionLoading] = createSignal(false)
  ...
  const runExpandedSearch = async (nextQuery: string, options?: { refreshSidecars?: boolean }) => {
    // The grounded AI answer, internal "i Velion" matches, images, and videos do
    ...
    if (refreshSidecars) {
      ...
      // "i Velion" arm queries the knowledge graph, and did-you-mean/related
  ...
  const fetchVelion = (nextQuery: string) => {
    velionController?.abort()
    const controller = new AbortController()
    velionController = controller
    setVelionLoading(true)
    setVelionResults([])
    searchNavbar(nextQuery, controller.signal)
      .then((payload) => {
        if (controller.signal.aborted) return
        setVelionResults(Array.isArray(payload.results) ? payload.results.slice(0, 6) : [])
      })
      .catch(() => {
        if (!controller.signal.aborted) setVelionResults([])
      })
      .finally(() => {
        if (!controller.signal.aborted) setVelionLoading(false)
      })
  }
  ...
  const submitFollowUp = (event: SubmitEvent) => {
    ...
    if (!next) return
    setFollowUpQuery('')
    if (webResults().length > 0 || webAnswer() || velionResults().length > 0) {
  ...
  const launchChatWithSearchContext = async (question?: string) => {
    const prompt = buildSearchChatPrompt({
      ...
      velion: velionResults().slice(0, 6),
    })
  ...
  return (
    <Show
      when={!props.expanded}
      fallback={
        <div class="velion-panel-in velion-search-expanded-shell">
              ...
              <Switch>
                ...
                <Match when={true}>
                  <div class="dashboard-xsearch-info">
                    ...
                    <VelionResultsSection loading={velionLoading()} results={velionResults()} />
      ...
      <div class="velion-panel-in relative transition-all duration-500 ease-out">
        <form onSubmit={submit} class="relative z-[1] w-full">
          ...
          <div class="dashboard-search-row">
            ...
            <div class="velion-glass-input dashboard-search-input-wrap">
          ...
          <Show when={showPreview()}>
            <div class="velion-fade-up dashboard-search-preview-results">
              <div>
                <Show
                  when={!previewLoading()}
...
function isSearchSnapshot(value: unknown): value is SearchSnapshot {
  ...
  return typeof record.query === 'string'
    ...
    && Array.isArray(record.velionResults)
...
function buildSearchChatPrompt(input: {
  ...
  if (input.velion.length > 0) {
    lines.push('', 'Velion internal matches:')
    for (const result of input.velion) {
      lines.push(`- ${result.label}${result.excerpt ? `: ${result.excerpt}` : ''}`)
    }
  }

  lines.push('', 'Use citations when possible. If the internal Velion matches are relevant, ground the answer in them before using public web sources.')
  return lines.join('\n')
}
