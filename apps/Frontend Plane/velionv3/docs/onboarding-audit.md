# Velion v3 Onboarding Audit

**Date:** 2026-06-10
**Scope:** `src/features/onboarding/**`, plus the auth feature, shared Velion primitives, `src/app/shell`, and `src/styles/global.css` where they interact with onboarding.
**Method:** Four audit lenses (bottlenecks, bad-code/redundancy, UI/UX, reuse), each finding then adversarially verified against the code (and where relevant against `velion-gateway-rs` and library internals). Every finding below survived that verification pass (`verdict.isReal = true`). 71 raw verified findings were deduplicated to **66 distinct findings** (5 cross-lens duplicates merged, keeping the higher severity). All file paths are relative to the `velionv3` root.

---

## Executive summary

The onboarding feature is structurally sound SolidJS: the route is lazy-loaded, props are read through getters, page-level derivations use `createMemo`, SSE progress writes are partially batched, and shared chrome correctly delegates to `src/shared/ui/velion` primitives. The flow is visually polished with genuinely good bones — resumable state, animated step transitions, inline busy labels on most async buttons.
...
2. **Language pill** — `OnboardingTopbar.tsx:41` renders `<VelionLanguageButton code="NB" />` with a chevron affordance and no onClick — while the auth feature wires the *same component* to a working locale toggle (`AuthFormPanel.tsx:39-44`), so users arriving from `/auth` expect it to work.
3. **Billing switch** — `PaywallStep.tsx:28` renders `<VelionSwitch label="Billing period" />` between "Månedlig"/"Årlig" with neither `checked` nor `onChange` (VelionSwitch accepts both). As an uncontrolled Kobalte switch the thumb visually toggles — appearing responsive — but `onboardingPlanCards` has monthly-only prices with a hardcoded `/mnd` suffix, so nothing ever changes.
...
`#a09890` on `#ffffff` is 2.84:1, far below the 4.5:1 AA threshold at the tiny sizes used: `.velion-text-button` — the interactive "Hopp over" skip actions — 11px (1898-1909); `.onboarding-field span` labels 11px; `.onboarding-eyebrow` 10px; connector group headings 10px; the "Legg til" row badge. The step pill `#7b746d` on `#f7f7f6` computes to exactly 4.30:1 at 11px — also failing. These include primary affordances, not just decoration; no WCAG exemption applies.

**Fix:** Darken to at least `#767069` (4.89:1 on white) — the interactive `.velion-text-button` especially; reserve `#a09890` for non-essential decoration. (Verifier: on the `#edebe7` card background `#767069` lands ~4.1:1 — go slightly darker there.)
...
Every step is Norwegian except AssemblyStep, which is fully English ("Final step · Assembly", "Velion is assembling the first workspace.", "pages grounded"/"sources started"/"launch plan") with one Norwegian button "Åpne dashboard". All user-facing `setError` fallback strings in OnboardingPage are English (245, 257, 301, 314, 355, 409), PaywallStep's switch label is "Billing period" (screen-reader-facing), and VelionStepDots aria-labels are English. The auth feature has a working nb/en copy system (`auth/lib/model.tsx` `getAuthCopy`) that onboarding ignores.
...
AuthPage implements content-height tracking manually (contentRef + createEffect + rAF + scrollHeight, lines 23, 26, 58-65) to animate the form panel, while the shared `createElementHeight` (`src/shared/ui/velion/createElementHeight.ts`) does this with a ResizeObserver and is used by OnboardingPage for the same purpose. The hand-rolled version only re-measures when `mode()` changes — verified to miss locale-toggle text changes the shared helper would catch.
...
**Fix:** Extract `createViewportCardScale(baseHeight, opts)` next to `createElementHeight` in `src/shared/ui/velion` (owning the resize listener via `onCleanup`), naming the 18px chrome inset and 0.52 floor as constants; compose the 1.04 multiplier on top in onboarding.
...
Two parallel CSS clusters implement the same "icon/prefix + borderless inner input inside a bordered row" control: `.onboarding-url-control` (1853-1888, WebsiteStep's Globe-prefixed URL field) and `.auth-field__control` (1000-1029, AuthFormPanel's three icon-led fields). Both sit inside VelionField, both strip the inner input's border. (Verifier: divergence is more than radii/colors — alignment, sizing, shadow — but all expressible as modifier skins.)

**Fix:** Extract a shared `VelionInputControl` (leading/trailing adornment + children input) with one `velion-input-control` cluster plus auth/onboarding modifier skins.
...
The signin/signup toggle uses raw buttons with string-concatenated classes (the only two template-literal class concats in all of `src/`) and no `aria-selected`/`aria-pressed` — worse, the wrapper declares `role="tablist"` while children lack `role="tab"` (invalid ARIA). The shared `VelionChoiceChip` shows the codebase's selected-toggle pattern (aria-pressed + `--selected` via `cn`).

**Fix:** Extract a small `VelionTabs` primitive (`role="tablist"` + `role="tab"`/`aria-selected`) in `src/shared/ui/velion`; at minimum switch to `classList`. (Verifier: direct VelionChoiceChip reuse does not work here — wrong ARIA semantics for tabs and conflicting base styles — prefer the tabs primitive.)
...
`AuthScreen.tsx:2` imports `onboardingFooterLinks` from `@/features/onboarding/lib/model` (`model.ts:167`) to feed shared `VelionScreen`/`VelionFooterLinks` primitives — verified to be the **only** cross-feature import in the entire codebase. The links ("Om oss", "Personvern", …) are app-wide chrome, not onboarding data.

**Fix:** Move the constant to `src/shared/ui/velion` (e.g. `velionFooterLinks`) and import it from both screens.

### R-13 — Shared Velion primitives carry `onboarding-*` class names and baked-in Norwegian copy
**Severity: Low** · `src/shared/ui/velion/VelionBackButton.tsx:20`

Five promoted primitives still emit feature-named classes: VelionBackButton `onboarding-back`, VelionLanguageButton `onboarding-language-pill`, VelionStepDots `onboarding-dots`, VelionStepPill `onboarding-step-pill`, VelionFooterLinks `onboarding-footer-links` — unlike the correctly named `velion-text-button`/`velion-icon-button`/`velion-switch`. Auth then restyles via override classes (`.auth-back-link`, `.auth-language-switcher`, `.auth-footer-links`). VelionStepPill also hardcodes "Steg {n} av {m}" inside a shared primitive.

**Fix:** Rename the CSS clusters to `velion-*` (the strings occur nowhere outside the five components and global.css — contained sed) and give VelionStepPill a `label`/format prop, matching VelionBackButton's existing overridable-label pattern.

### R-14 — `OnboardingLinkButton`'s `'link'` emphasis value is a dead alias adding a mapping layer over VelionTextButton
**Severity: Low** · `src/features/onboarding/components/shared/OnboardingLinkButton.tsx:15`

The wrapper exists solely to remap emphasis `'link' | 'large'` to `'default' | 'large'`. No call site ever passes `emphasis="link"` (only `emphasis="large"` at OrganizationStep:48; all others pass none), so the `'link'` member and the ternary are dead; `undefined` already behaves as `'default'` in VelionTextButton. Similarly `OnboardingField` only adds the static class `'onboarding-field'` to VelionField.

**Fix:** Delete OnboardingLinkButton and use VelionTextButton directly (verified drop-in at all five call sites), or align the union and pass through. Keep OnboardingField only if more behavior is planned.
...
| R-04 | AuthPage rAF height measurement | **Reuse** `src/shared/ui/velion/createElementHeight.ts` |
| R-05 | viewportHeight + cardScale (Auth & Onboarding) | **Extract** `createViewportCardScale(baseHeight, opts)` into `src/shared/ui/velion` |
...
| R-09 | `.auth-field__control` / `.onboarding-url-control` | **Extract** `VelionInputControl` in `src/shared/ui/velion` |
| R-10 | Auth signin/signup tabs | **Extract** `VelionTabs` (proper `role="tab"`/`aria-selected`) in `src/shared/ui/velion` |
| R-11 | Crawl phase list vs assembly checklist | **Extract** `OnboardingChecklist`; delete `.onboarding-assembly-list` |
| R-12 | `onboardingFooterLinks` cross-feature import | **Move** constant to `src/shared/ui/velion` as `velionFooterLinks` |
| R-13 | `onboarding-*` classes in 5 shared primitives | **Rename** to `velion-*`; add label prop to `VelionStepPill` |
| R-14 | `OnboardingLinkButton` dead alias | **Delete**; use `VelionTextButton` directly |
