# velionv3 Gateway Integration Plan

**Architecture decision (FIXED):** velionv3 (SolidJS SPA, no BFF, no server tier) talks ONLY to
`velion-gateway-rs` (Frontend Plane, `apps/Frontend Plane/velionv3/apps/gateway/`, HTTP :3185).
...
internal "i Velion" arm reuses `GET /api/v1/navbar/search?scope=knowledge` (retrieval-engine). All
...
`VelionIngestionsPage`): scrape/crawl/extract/batch runs, schedules + lifecycle, browser profiles,
web sources, and the per-run evidence timeline. Port of velionv2's `app/api/ingestions/` BFF.
...
> token's `claims.org_id`, so this domain never reads `x-velion-org-id`.
>
...
headers; dev headers behind `ALLOW_DEV_ACTOR_HEADERS`; silent fallback actor `velion-v3-local-user`.
...
5. **Dev mode**: keep `ALLOW_DEV_ACTOR_HEADERS` but make it fail-closed in release builds
   (`#[cfg(debug_assertions)]` or explicit `VELION_ENV=dev` check) and **remove the silent
   `velion-v3-local-user` fallback** — unauthenticated requests get 401
...
- CORS: `VELION_ALLOWED_ORIGINS` must list the real SPA origins per environment (dev
...
| New file | Contents |
|---|---|
| `src/shared/api/config.ts` | `gatewayBaseUrl()` from `VITE_VELION_GATEWAY_URL` (fallback `http://127.0.0.1:3185`), env typing in `src/vite-env.d.ts`; single source — `src/features/onboarding/lib/api.ts` migrates onto it |
...
- `src/shared/session/session-store.ts`: Solid store holding `{user, activeOrg, entitlements, status}`,
  ...
  (`demoActor` `user_demo_operator`/`org_triodelab` in `src/shared/mocks/velion-operating-model.ts`
...
### C.3 Mock replacement map (`src/shared/mocks/velion-operating-model.ts` shrinks per phase)
...
| `workflowPolicies`, `selectModelTier()` | settings-client (+ AI settings section) | 4 |
...
**Gateway tasks (Rust, `velion-gateway-rs/`):**
...
**v3:** `settings-client.ts` + `notifications-client.ts`; replace `workflowPolicies`/`selectModelTier()`
(model tier → `PUT /api/v1/settings/ai`; workflow policies → settings-backed interim store until a
dedicated policy service exists — `workflows.toggle_policy` action routes there); org/member/role
