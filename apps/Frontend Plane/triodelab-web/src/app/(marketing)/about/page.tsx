...
function MaskedLine({
  ...
  return (
    <span className={`relative block ${className}`}>
      <m.span className="relative block will-change-transform" style={{ opacity }}>
        {children}
      </m.span>
    </span>
  );
...
function HighlightText({ text, highlight }: { text: string; highlight: string }) {
  ...
  return (
    <>
      {parts.map((p, idx) => (
        <React.Fragment key={`hl-${idx}-${p.slice(0, 8)}`}>
          {p}
          {idx < parts.length - 1 && (
    ...
    >
      ...
      <div className="mx-auto mt-4 sm:mt-6 max-w-xl overflow-clip text-sm leading-relaxed text-neutral-600 md:text-base">
        <MaskedLine t={t} delay={0.08} className="text-balance">
          <HighlightText text={chapter.text} highlight={chapter.highlight} />
        </MaskedLine>
      </div>
