import type { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Agenci – AI Chatbot Platform | Triodelab',
  description: 'Agenci er en moderne AI-drevet chatbot-plattform fra Triodelab. Intelligent automatisering og saksbehandling – kommer snart.',
}

export default function AgenciPage() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#F4F1EB]">
      <div className="text-center">
        <h1 
          className="mb-4 text-6xl font-light tracking-wide text-[#111111]"
          style={{ fontFamily: 'var(--font-cormorant-garamond), serif' }}
        >
          Agenci
        </h1>
        <p className="text-[#6A655F]" style={{ fontFamily: 'Inter, sans-serif' }}>
          Chatbot Platform
        </p>
        <p className="mt-8 text-sm text-[#6A655F]" style={{ fontFamily: 'Inter, sans-serif' }}>
          Coming Soon
        </p>
      </div>
    </div>
  );
}
