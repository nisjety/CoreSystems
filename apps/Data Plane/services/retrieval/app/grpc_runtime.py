...
PROTO_FILES = ("retrieval.proto", "auth.proto", "org_access.proto")
...
def ensure_proto_generated() -> None:
    expected_outputs = (
        PROTO_GEN_DIR / "retrieval_pb2.py",
        PROTO_GEN_DIR / "retrieval_pb2_grpc.py",
    )
...
def load_retrieval_proto_modules() -> tuple[ModuleType, ModuleType]:
    ensure_proto_generated()
    return (
        importlib.import_module("retrieval_pb2"),
        importlib.import_module("retrieval_pb2_grpc"),
    )