...
impl KernelDriver {
    ...
    async fn ensure_browser(&self, lease: &BrowserLease) -> QuarryResult<String> {
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                QuarryError::new(ErrorCode::DriverFailed, format!("kernel create failed: {e}"))
            })?;
        ...
        let parsed: CreateBrowserResponse = resp.json().await.map_err(|e| {
            QuarryError::new(ErrorCode::DriverFailed, format!("kernel decode failed: {e}"))
        })?;
