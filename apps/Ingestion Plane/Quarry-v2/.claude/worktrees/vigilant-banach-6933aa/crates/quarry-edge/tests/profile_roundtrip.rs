...
async fn profile_save_load_roundtrip_preserves_all_fields() {
    ...
    let save_resp = client
        ...
        .await
        .unwrap();
    assert!(save_resp.status().is_success(), "save failed: {save_resp:?}");
    let save_body: Value = save_resp.json().await.unwrap();
    let profile_id = save_body["data"]["profile_id"].as_str().unwrap().to_string();

    // Load
    let load_resp = client
        .get(format!("http://{addr}/v1/profiles/{profile_id}"))
        .send()
        .await
        .unwrap();
...
async fn profile_list_includes_saved_profile() {
    ...
    let list_resp = client
        .get(format!("http://{addr}/v1/profiles"))
        .send()
        .await
        .unwrap();
    ...
    assert!(profiles.contains(&saved_id), "{profiles:?} missing {saved_id}");
}
...
async fn profile_delete_removes_entry() {
    ...
    let del_resp = client
        .delete(format!("http://{addr}/v1/profiles/{saved_id}"))
        .send()
        .await
        .unwrap();
    ...
    let load_resp = client
        .get(format!("http://{addr}/v1/profiles/{saved_id}"))
        .send()
        .await
        .unwrap();
...
async fn profile_load_unknown_returns_404() {
    ...
    let resp = client
        .get(format!("http://{addr}/v1/profiles/{unknown}"))
        .send()
        .await
        .unwrap();
...
async fn profile_save_with_specified_id_round_trips_that_id() {
    ...
    let load = client
        .get(format!("http://{addr}/v1/profiles/{chosen_str}"))
        .send()
        .await
        .unwrap();
