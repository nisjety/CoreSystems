    ...
    T: serde::de::DeserializeOwned,
{
    ...
    if let Some(signer) = state.internal_signer.as_ref() {
        ...
        let signed =
            crate::internal_auth::apply_to_request(signer, "GET", &path_q, b"");
        req = req
...
pub async fn list_snapshots(
    ...
    let filter = q.into_filter().map_err(|e| err_response(&request_id, e))?;
    let page =
        forward_list::<Snapshot>(&state, &request_id, &claims.org_id, "/v1/snapshots", &filter)
            .await
            .map_err(|e| err_response(&request_id, e))?;
    Ok(Json(Envelope::ok(request_id, page)))
}
...
) -> Result<Json<Envelope<Page<quarry_core::job_history::JobHistoryEvent>>>, (StatusCode, Json<Envelope<()>>)> {
    let request_id = RequestKind::new().to_string();
    let parsed_run_id: quarry_core::ids::kinds::RunKind = run_id.parse().map_err(|e: quarry_core::error::QuarryError| {
        err_response(
            &request_id,
