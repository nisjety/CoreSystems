...
use async_graphql::{Context, EmptyMutation, EmptySubscription, Object, Schema, SimpleObject, InputObject};
...
mod tests {
    ...
    fn schema_introspection_contains_jobs_query() {
        ...
        assert!(sdl.contains("type JobPage"), "JobPage type missing from SDL");
        assert!(sdl.contains("input ListFilterInput"), "ListFilterInput missing");
    }
    ...
    async fn version_query_resolves() {
        ...
        assert!(result.errors.is_empty(), "version query failed: {:?}", result.errors);
    ...
    async fn jobs_query_with_auth_returns_empty_page() {
        ...
        let schema = build_schema();
        let req = async_graphql::Request::new("{ jobs { items { jobId } totalEstimated } }")
            .data(claims);
