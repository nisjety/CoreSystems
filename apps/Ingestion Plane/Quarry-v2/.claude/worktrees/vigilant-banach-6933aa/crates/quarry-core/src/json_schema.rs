...
pub enum ValidationIssue {
    ...
    EnumMismatch { path: String, allowed: Vec<Value>, got: Value },
    LengthOutOfRange { path: String, len: usize, min: Option<usize>, max: Option<usize> },
    NumberOutOfRange { path: String, value: f64, min: Option<f64>, max: Option<f64> },
    AdditionalProperty { path: String, key: String },
...
impl ValidationIssue {
    ...
    pub fn human(&self) -> String {
        match self {
            ...
            ValidationIssue::TypeMismatch { path, expected, got } => {
            ...
            ValidationIssue::LengthOutOfRange { path, len, min, max } => {
                format!("{path}: length {len} outside bounds min={min:?} max={max:?}")
            }
            ValidationIssue::NumberOutOfRange { path, value, min, max } => {
...
fn validate_at(path: &str, value: &Value, schema: &Value, issues: &mut Vec<ValidationIssue>) {
    ...
    match value {
        ...
        Value::Array(arr) => {
            let min = schema_obj.get("minItems").and_then(|v| v.as_u64()).map(|n| n as usize);
            let max = schema_obj.get("maxItems").and_then(|v| v.as_u64()).map(|n| n as usize);
        ...
        Value::String(s) => {
            let min = schema_obj.get("minLength").and_then(|v| v.as_u64()).map(|n| n as usize);
            let max = schema_obj.get("maxLength").and_then(|v| v.as_u64()).map(|n| n as usize);
