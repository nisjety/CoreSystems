...
pub fn merge_preset_with_overrides(
    preset: Preset,
...
pub fn validate_preset_compatibility(
    ...
    crate::output_profile::validate_preset_compatibility(
        &merged.output,
...
mod tests {
    ...
    fn presets_have_unique_names() {
        ...
        sorted.sort();
        sorted.dedup();
        assert_eq!(names.len(), sorted.len(), "duplicate preset names: {names:?}");
    }
    ...
    fn merge_overrides_replaces_specified_fields() {
        ...
        let merged = merge_preset_with_overrides(
            p,
            &json!({ "crawl": { "max_depth": 10 } }),
        );
    ...
    fn validate_rejects_zero_max_pages() {
        let p = resolve_preset("docs-site").unwrap();
        let err = validate_preset_compatibility(&p, &json!({"crawl": {"max_pages": 0}}))
            .unwrap_err();
    ...
    fn validate_rejects_runaway_depth() {
        let p = resolve_preset("docs-site").unwrap();
        let err = validate_preset_compatibility(&p, &json!({"crawl": {"max_depth": 100}}))
            .unwrap_err();
