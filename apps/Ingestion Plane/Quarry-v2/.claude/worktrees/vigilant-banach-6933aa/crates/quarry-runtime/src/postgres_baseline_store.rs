...
use quarry_core::change_history::{
    BaselineSnapshot, ChangeRecord, ChangeStatus, DiffRecord,
};
...
mod tests {
    ...
    fn baseline(org: &str, url: &str, fp: &str, prev: Option<&str>) -> BaselineSnapshot {
        BaselineSnapshot {
            baseline_id: format!("bln_{}", quarry_core::ids::Id::<quarry_core::ids::kinds::ArtifactKind>::new()),
            org_id: org.into(),
            source_url: url.into(),
            fingerprint: fp.into(),
            artifact_id: None,
    ...
    async fn save_then_load_latest_returns_most_recent() {
        ...
        let latest = store.load_latest("org_a", "https://x/").await.unwrap().unwrap();
        assert_eq!(latest.fingerprint, "blake3:b");
        assert_eq!(latest.prev_baseline_id.as_deref(), Some(first.baseline_id.as_str()));
    }
    ...
    async fn compare_snapshot_returns_correct_status() {
        ...
        // No baseline yet → New.
        let r = store.compare_snapshot("org_a", "https://x/", "fp_new").await.unwrap();
        ...
        let r = store.compare_snapshot("org_a", "https://x/", "fp_v1").await.unwrap();
        assert_eq!(r.status, ChangeStatus::Unchanged);

        // Different fingerprint → Changed.
        let r = store.compare_snapshot("org_a", "https://x/", "fp_v2").await.unwrap();
