...
impl NatsEventBus {
    ...
    pub async fn connect(config: NatsConfig) -> QuarryResult<Self> {
        let mut opts = ConnectOptions::new()
            .connection_timeout(Duration::from_secs(config.connect_timeout_s));
    ...
    pub async fn ensure_stream(&self) -> QuarryResult<()> {
        ...
        self.js.get_or_create_stream(stream_cfg).await.map_err(|e| {
            QuarryError::new(
                ErrorCode::DriverFailed,
...
impl EventBus for NatsEventBus {
    async fn publish(
        ...
        let bytes = serde_json::to_vec(&event).map_err(|e| {
            QuarryError::new(
                ErrorCode::Internal,
                format!("event serialize failed: {e}"),
            )
        })?;
        ...
        let primary_pub = self.js.publish(primary_subject.clone(), payload_bytes.clone());
...
mod tests {
    ...
    async fn next_seq_is_monotonic_per_run() {
        ...
        async fn next(
            ...
            let c = m.entry(key.to_string()).or_insert_with(|| Arc::new(AtomicU64::new(0)));
