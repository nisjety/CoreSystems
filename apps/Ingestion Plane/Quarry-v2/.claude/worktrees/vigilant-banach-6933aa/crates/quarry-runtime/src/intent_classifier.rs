...
mod tests {
    ...
    async fn rule_classifier_matches_free_function() {
        ...
        assert_eq!(c.classify("how does tokio work").await, QueryIntent::Default);
    }
    ...
    fn parses_clean_tokens() {
        ...
        assert_eq!(MpIntentClassifier::parse("COMPARE"), QueryIntent::Comparative);
    ...
    async fn hybrid_short_circuits_on_obvious_intents() {
        ...
        assert_eq!(hybrid.classify("github.com").await, QueryIntent::Navigational);
    ...
    async fn cached_returns_same_intent_on_hot_key() {
        ...
        assert_eq!(cached.classify("rust borrow checker").await, QueryIntent::Research);
        assert_eq!(cached.classify("rust borrow checker").await, QueryIntent::Research);
        assert_eq!(cached.classify("rust borrow checker").await, QueryIntent::Research);
