...
fn merge_dedupe(
    ...
    let mut seen: std::collections::HashSet<String> =
        base.iter().map(|r| r.url.clone()).collect();
    for r in extra {
...
const FRESH_KEYWORDS: &[&str] = &[
    "today", "yesterday", "tomorrow", "now", "latest", "current",
    "breaking", "live", "this week", "this month", "this year",
    "recent", "news",
];
...
mod tests {
    ...
    fn classifies_year_2026_as_fresh() {
        assert_eq!(
            classify_intent("rust roadmap 2026"),
            QueryIntent::Fresh
        );
    }
    ...
    fn classifies_neutral_as_default() {
        assert_eq!(
            classify_intent("how does tokio work"),
            QueryIntent::Default
        );
        assert_eq!(
    ...
    fn merge_dedupe_keeps_first_occurrence() {
        let base = vec![r("https://a/", "tantivy_local"), r("https://b/", "tantivy_local")];
        ...
        assert!(out.iter().any(|r| r.url == "https://c/" && r.provider == "stract"));
    }
    ...
    async fn cache_returns_same_results_within_ttl() {
        ...
        let router = SmartSearchRouter::builder()
            ...
            .build()
            .unwrap();
        let r1 = router.search("rust", &SearchOptions::default()).await.unwrap();
        let r2 = router.search("rust", &SearchOptions::default()).await.unwrap();
