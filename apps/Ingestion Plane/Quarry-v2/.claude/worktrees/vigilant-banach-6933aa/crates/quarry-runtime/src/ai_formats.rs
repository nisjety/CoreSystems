...
impl AiFormatRunner {
    ...
    async fn invoke(&self, content: &str) -> QuarryResult<crate::mp_client::ModelPlaneInvokeResponse> {
...
mod tests {
    ...
    async fn json_invalid_when_required_missing() {
        ...
        Mock::given(method("POST"))
            .and(path("/v1/invoke"))
            .respond_with(ResponseTemplate::new(200).set_body_json(mp_response("{\"name\": \"A\"}")))
            .mount(&server)
            .await;
