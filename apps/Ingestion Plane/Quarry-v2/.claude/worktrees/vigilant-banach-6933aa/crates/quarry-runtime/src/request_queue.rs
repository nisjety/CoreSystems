...
impl RequestQueue for InMemoryRequestQueue {
    async fn enqueue(
        ...
        if let Some(cap) = self.max_size {
            let total: usize = s.by_priority.values().map(|q| q.len()).sum::<usize>()
                + s.in_flight.len();
            if total >= cap && !s.known.contains(&request_id) {
                return Err(QuarryError::new(
                    ErrorCode::RateLimited,
                    format!(
                        "request queue full (cap={cap}, current={total}); enqueue rejected"
                    ),
                ));
            }
        }
...
mod tests {
    ...
    async fn enqueue_pop_ack_roundtrip() {
        ...
        q.enqueue("r1".into(), "https://x".into(), Priority::Default, json!({}))
            .await
            .unwrap();
    ...
    async fn enqueue_is_idempotent_on_request_id() {
        let q = InMemoryRequestQueue::default();
        let first = q
            .enqueue("dup".into(), "https://x".into(), Priority::Default, json!({}))
            .await
            .unwrap();
        let second = q
            .enqueue("dup".into(), "https://x".into(), Priority::Default, json!({}))
    ...
    async fn high_priority_pops_before_default() {
        let q = InMemoryRequestQueue::default();
        q.enqueue("low".into(), "u".into(), Priority::Low, json!({})).await.unwrap();
        q.enqueue("def".into(), "u".into(), Priority::Default, json!({})).await.unwrap();
        q.enqueue("hi".into(), "u".into(), Priority::High, json!({})).await.unwrap();
    ...
    async fn reap_expired_requeues_in_flight_after_timeout() {
        ...
        q.enqueue("r".into(), "u".into(), Priority::Default, json!({})).await.unwrap();
    ...
    async fn fail_permanently_drops_request_silently() {
        let q = InMemoryRequestQueue::default();
        q.enqueue("doomed".into(), "u".into(), Priority::Default, json!({})).await.unwrap();
    ...
    async fn enqueue_rejects_when_max_size_exceeded() {
        ...
        q.enqueue("a".into(), "u".into(), Priority::Default, json!({})).await.unwrap();
        q.enqueue("b".into(), "u".into(), Priority::Default, json!({})).await.unwrap();

        // Re-enqueue of existing id is still idempotent (returns false, doesn't reject).
        let ret = q.enqueue("a".into(), "u".into(), Priority::Default, json!({})).await.unwrap();
        assert!(!ret);
