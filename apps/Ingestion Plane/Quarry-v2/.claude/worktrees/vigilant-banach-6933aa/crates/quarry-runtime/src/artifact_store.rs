...
impl ArtifactStore for InMemoryStore {
    ...
    async fn list(
        ...
        let mut window: Vec<&InMemoryArtifactMeta> =
            rows.iter().take(limit + 1).copied().collect();
...
mod tests {
    ...
    async fn in_memory_list_cursor_paginates_correctly() {
        ...
        let p1_ids: std::collections::HashSet<_> =
            page1.items.iter().map(|m| m.artifact_id.to_string()).collect();
        let p2_ids: std::collections::HashSet<_> =
            page2.items.iter().map(|m| m.artifact_id.to_string()).collect();
        assert!(
