...
mod tests {
    ...
    async fn end_to_end_returns_answer_with_citations() {
        ...
        Mock::given(method("POST"))
            .and(path("/v1/invoke"))
            .respond_with(ResponseTemplate::new(200).set_body_json(mp_response(
                "Rust's async ecosystem centers on Tokio.",
            )))
            .mount(&server)
            .await;
    ...
    async fn all_fetches_fail_returns_citations_only() {
        ...
        let result = pipeline
            ...
            .await
            .unwrap();
        assert!(result.answer.is_empty(), "no synthesis when no fetches succeed");
    ...
    async fn closure_fetcher_works() {
        let cf = ClosureFetcher(|url: String, _zdr: ZdrMode| async move {
            Some(format!("mock for {url}"))
        });
        let md = cf
