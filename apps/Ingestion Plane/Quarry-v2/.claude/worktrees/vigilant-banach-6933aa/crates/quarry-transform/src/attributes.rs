    ...
    for cap in meta_re.captures_iter(html) {
        ...
        if let Some(name) = attrs.get("name") {
            ...
            if lower.starts_with("dc.")
                ...
                || lower == "author"
            {
                out.entry(name.to_string()).or_insert_with(|| content.to_string());
            }
        }
        ...
        if let Some(prop) = attrs.get("property") {
            ...
            if lower.starts_with("og:") || lower.starts_with("article:") {
                out.entry(prop.to_string()).or_insert_with(|| content.to_string());
            }
        }
    }
    out
...
mod tests {
    ...
    fn strips_scripts_and_noscript() {
        let html = "<html><body><p>ok</p><script>alert(1)</script><noscript>nope</noscript></body></html>";
    ...
    fn strips_tracking_pixels() {
        let html = r#"<html><body><img src="a.gif" width="1" height="1"><img src="b.gif"></body></html>"#;
        let out = strip_scripts(html);
    ...
    fn skips_table_with_existing_thead() {
        let html = "<table><thead><tr><th>X</th></tr></thead><tbody><tr><td>v</td></tr></tbody></table>";
        let out = normalize_tables(html);
    ...
    fn resolves_relative_anchor_hrefs() {
        let html = r##"<a href="/path">x</a><a href="https://x.test/abs">y</a><a href="#frag">z</a>"##;
