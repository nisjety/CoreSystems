...
use quarry_transform::{
    fingerprint::content_fingerprint, links, metadata,
...
fn run_fixture(entry: &FixtureEntry, fixtures_dir: &Path) -> FixtureResult {
    ...
    for phrase in &entry.expected.must_not_contain {
        ...
        checks.push(Check {
            ...
            detail: if pass {
                ...
                Some(format!("'{}' found in markdown output (boilerplate leak)", phrase))
            },
        });
    }
    ...
    checks.push(Check {
        ...
        detail: if link_pass {
            ...
            Some(format!("found {} links, expected >= {}", link_list.len(), entry.expected.min_links))
        },
    });

    let link_hrefs: Vec<&str> = link_list.iter().map(|l| l.href.as_str()).collect();
    for expected_link in &entry.expected.link_must_contain {
        let pass = link_hrefs.iter().any(|h| h.contains(expected_link.as_str()));
        checks.push(Check {
...
fn main() {
    ...
    if args.iter().any(|a| a == "scoreboard") {
        ...
        println!("scoreboard: wrote {} entries to {}", entries.len(), out.display());
        return;
    }
    ...
    for entry in &manifest.fixtures {
        ...
        println!(
            "[{}] {} ({}) — {}ms, {} bytes, {} links",
            status, result.id, result.category, result.duration_ms, result.markdown_bytes, result.link_count
        );
