...
use chrono::Utc;
use quarry_core::benchmark::{
    BenchmarkSuite, BucketScore, MetricTarget, ScorecardEntry,
};
...
fn run_quarry_v2(suite: &BenchmarkSuite, dry_run: bool) -> ScorecardEntry {
    let sample_count = if dry_run { 0 } else { suite.corpus.len() as u32 };
...
mod tests {
    ...
    fn dry_run_empty_corpus_emits_valid_scorecard_entry() {
        ...
        assert!(entries[0].artifact_url.as_deref().unwrap().starts_with("dry-run"));
