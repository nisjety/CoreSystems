        ...
        for provider in &cfg.providers {
            ...
            row_futures.push(async move {
                ...
                let endpoint_futures: Vec<_> = endpoints
                    ...
                    .collect();
                let captures: Vec<EndpointCapture> = futures::future::join_all(endpoint_futures).await;
...
async fn capture_via_browserless(
    ...
    let body: serde_json::Value = serde_json::from_str(extract_json_from_html(&text))
...
async fn capture_via_kernel(
    ...
    let body: serde_json::Value = serde_json::from_str(extract_json_from_html(&body_text))
        .unwrap_or(serde_json::Value::Null);
