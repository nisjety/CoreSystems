...
**Status:** Architecture, standards, research, implementation proposal, and ownership-reconciled target design  
...
## 1b. Ownership reconciliation — 2026-08-03

The earlier CoreSystem ownership matrix remains directionally correct, but this
verified Quarry audit exposes four places where the target design must be stated
more precisely.

1. **Approval continuation is not approval authority.** Quarry must persist a
   pending browser step and resume it only after receiving an exact, signed
   authorization. Application/Conversation owns the durable human approval
   record; Model Plane owns risk classification and the request for approval;
   BrowserBroker issues/revokes the exact browser grant; Quarry only validates
   and consumes it.
2. **Planner-model routing is not runtime routing.** Model Plane selects the
   planner/verifier model. Quarry selects the acquisition driver, browser
   backend, session, proxy, observation representation, and deterministic
   recovery path within the granted policy and budget.
3. **Search/answer product facades do not transfer reasoning ownership.** The
   existing `SmartSearchRouter` is a legitimate Quarry acquisition component.
   The existing `AnswerPipeline` is current implementation reality, but final
   answer synthesis, research-loop continuation, and semantic evidence
   sufficiency remain Model Plane responsibilities. Preserve a compatibility
   endpoint only as a delegated facade while moving model reasoning behind the
   Model Plane contract.
4. **Operational browser memory is not Data Plane knowledge.** Quarry may own
   `SiteReliabilityProfile`, `ElementFingerprint`, runtime manifests, action
   traces, compiled browser procedures, and proof receipts because they are
   execution-control metadata. Source documents, chunks, embeddings, retrieval
   indexes, graphs, wiki pages, and promoted durable knowledge remain Data Plane
   owned.

The authority chain for an effectful browser step is therefore:

```text
Control Plane identity/membership attestation
  + Application/Conversation durable intent or approval decision
  + Model Plane risk classification and exact proposed effect
  -> BrowserBroker exact, tenant-bound, short-lived grant
  -> Quarry validates grant and executes one typed browser effect
  -> Quarry produces deterministic receipt / UNKNOWN
  -> provider or Integration read-back when an external API is authoritative
```

Quarry must fail closed when any required authority reference is absent,
expired, mismatched, revoked, or broader than the exact effect being executed.
...
```text
Control Plane
  - owns canonical user identity and exact organization membership
  - issues/verifies service and actor authority attestations
  - is not replaced by tenant headers, session hints, or local projections

Application Plane / Conversation
  - owns durable human intent and approval decisions
  - owns the approval request/decision record and application delivery ledger
  - exposes the human-facing API consumed by the App Shell

Model Plane
  - interprets the task
  - creates browser/research plans
  - selects planner and semantic-verifier models
  - chooses from Quarry observations
  - decides whether to continue, replan, or stop
  - classifies risk and determines when approval is required
  - does not itself grant browser authority

BrowserBroker
  - issues tenant-, actor-, run-, and effect-bound browser grants
  - restricts domains, action classes, credentials, budgets, and expiry
  - binds an exact approval/intent reference where required
  - validates and revokes browser authority

Quarry V2
  - chooses the acquisition driver and browser runtime backend
  - manages browser execution leases and operational session state
  - creates observations
  - executes typed browser actions under a validated grant
  - enforces browser/runtime/network security
  - verifies deterministic postconditions
  - stores execution evidence, artifacts, traces, and operational browser metadata
  - persists pending execution continuation without originating approval
  - emits complete action history and outcome receipts

Integration / provider execution
  - owns non-browser provider API execution and provider-specific receipts
  - may provide authoritative read-back used to verify a browser-caused effect
  - does not accept Quarry or Model Plane intent without the exact durable authorization contract

Data Plane
  - owns promoted source versions, documents, chunks, embeddings, retrieval indexes,
    graphs, wiki pages, and other durable knowledge assets
  - may ingest Quarry evidence through an explicit API; Quarry never crosses its database boundary
...
SolidJS App Shell
  ...
  - approval and intent UI backed by Application/Conversation
  - intervention and takeover UI
  - task timeline and proof display
  - never acts as identity, approval, or browser authority by itself
```

Model Plane must not directly control Browserbase, Chromium, Playwright, or provider credentials. All browser execution goes through Quarry's trusted action boundary, and every effectful action must be backed by the exact Control/Application/BrowserBroker authority chain described above.
...
Record which resolver succeeded. This becomes site/browser-procedure reliability data.
...
```text
B0 deterministic compiled browser procedure
...
```text
Known compiled browser procedure?
...
## 13. Action caching and browser-procedure compilation

Repeated successful agent behavior should become a deterministic program.

### 13.1 Candidate browser procedure

```yaml
procedure_id: vendor.download_latest_invoice
version: 5
...
successful agent trajectory
  ...
  -> security review
  -> candidate browser procedure
  -> approved browser procedure
```
...
that exists today is `crawl_signals.rs`'s operator pause/resume/cancel of an
**entire crawl run** at the page boundary.

The missing Quarry work is to persist an exact pending action, release or
suspend execution resources safely, and resume only after BrowserBroker
returns a new or continued grant bound to the same tenant, actor, run, action,
payload digest, idempotency key, and Application/Conversation approval record.
Quarry must never create, infer, broaden, or substitute the approval decision.
...
When a human takes control, Quarry marks the lease as human-controlled and rejects new agent actions. Model Plane must not continue issuing actions until control is explicitly returned through an authenticated Application/Conversation event and BrowserBroker refreshes the executable grant.
...
Detect login/session changes and invalidate the site profile or browser-procedure cache when:
...
actual answer/search routes, not a parallel or aspirational stack. **Two
ownership conclusions follow.** First, neither has any connection to
`quarry-browser`/`action_runtime.rs` today — "should integrate" describes
real, not-yet-done wiring. Second, `SmartSearchRouter` is correctly Quarry-owned
acquisition logic, while model-generated answer synthesis is not a long-term
Quarry authority. Treat the current `AnswerPipeline` as implementation reality
and a compatibility bridge: preserve the route if needed, but move planner,
semantic evidence sufficiency, citation-aware synthesis, and continuation
behind the Model Plane contract.

Quarry should integrate SmartSearchRouter, browser acquisition, and evidence
packaging into one web-intelligence execution path. Model Plane should consume
that path for agentic research and final cited synthesis rather than Quarry
creating a separate autonomous research authority.
...
Expose the high-level endpoint through the Application/API or Model Plane
gateway, not as a Quarry-owned reasoning endpoint:

```text
POST /v1/agent
  -> Application/API authenticates and resolves organization authority
  -> Model Plane plans, selects models, and owns the research loop
  -> BrowserBroker grants exact browser capabilities
  -> Quarry performs search/fetch/browser execution and returns proof artifacts
  -> Model Plane synthesizes the cited result
```

A compatibility route may temporarily be mounted at `quarry-edge`, but it must
be a thin delegated facade with no local agent loop, independent model routing,
...
Firecrawl's simplicity is a competitive advantage. Keep low-level acquisition
and browser execution in the Quarry SDK, while exposing reasoning through the
product/Model Plane SDK:
...
velion.answer(); // Model Plane synthesis over Quarry evidence
velion.agent();  // Model Plane task/agent facade
```

A unified top-level client may hide REST/gRPC/NATS implementation details, but
its namespace and internal routing must preserve ownership rather than making
`quarry.answer()` or `quarry.agent()` independent reasoning authorities.
...
interface SiteProfile {
	...
	compiledBrowserProcedures: string[];
...
Keep browser-agent action sequences serial within one tab unless the browser procedure explicitly supports parallel pages.
...
Every browser run creates a shared routing example:

```text
site/task state
+ requested and selected observation mode
+ Model Plane planner/verifier model IDs
+ Quarry-selected acquisition driver/runtime backend
+ action sequence
+ failures/recovery
+ verified outcome
+ browser/network/model cost and latency components
= cross-plane browser routing record
```

Ownership of learning is split deliberately:

**Quarry learns or configures:**

- which acquisition driver/runtime works for a host and task class;
- which sites need screenshots or richer observations;
- when Browserbase or another runtime improves verified success enough to justify cost;
- which selectors/fingerprints remain stable;
- which proxy/session profiles work;
- which action types produce unknown outcomes.

**Model Plane learns or configures:**

- which planner/verifier model works for a task class;
- when to use structured, visual, frontier, or human planning;
- whether a trajectory should remain agentic or be proposed for compilation;
- semantic recovery and replanning policy.

Start both sides with explicit rules and offline ranking. A LightGBM/ranker-style
model for Quarry runtime selection may be deployed only inside the Quarry
execution boundary; planner-model routing belongs in Model Plane
`capability-core`/eval infrastructure. Do not add another LLM merely to route
browser backends.
...
### `quarry-edge` ownership correction

`quarry-edge` may expose public REST/SSE for acquisition, browser sessions,
artifacts, and compatibility facades. It must not become the canonical owner of
agent planning, model selection, final answer synthesis, human approval, or
identity. Existing answer routes should delegate synthesis to Model Plane while
preserving Quarry-owned source capture, citation evidence, and transport
compatibility.
...
- durable `SiteReliabilityProfile` resources;
- compiled **browser procedure** registry and rollout state;
- action-cache metadata;
- replay/artifact/proof manifests;
- runtime and execution benchmark results;
- pending browser-step continuation records that reference external authority;
- lifecycle reconciliation.

`quarry-control` stores execution-control state. It does not own human approval
decisions, planner-model policy, canonical identity, or durable knowledge.

### `quarry-orchestrator`

Add Temporal workflows for:

- browser execution supervision;
- wait/resume on an exact externally issued BrowserBroker authorization;
- human takeover timeout;
- session reconnect;
- browser-procedure promotion tests;
- runtime/driver benchmark runs;
- profile health checks;
- replay retention/deletion.

The orchestrator may wait for and consume approval signals, but must not create
or reinterpret the underlying approval decision.

### Control Plane identity authority

Add or require:

- exact actor and organization membership attestation;
- service-to-service audience and tenant binding;
- revocation/fail-closed semantics;
- no trust in caller-supplied organization headers or Application projections.

### Application Plane / Conversation approval authority

Add or require:

- durable user intent and approval request/decision records;
- exact action/effect/payload digest and idempotency binding;
- authenticated approval and takeover events for the App Shell;
- cancellation and expiry semantics;
- immutable references consumable by BrowserBroker and Quarry proof receipts.

### Model Plane `browser-broker`

Add:

- durable, short-lived grants;
- tenant, actor, run, graph-node, action, and payload-digest binding;
- domain and action class restrictions;
- Quarry lease binding;
- exact credential references;
- exact Application/Conversation approval reference where required;
- grant revocation events;
- browser runtime budget metadata.
...
- approval requirement generation without approval issuance;
...
- provider-specific read-back capability for authoritative outcome verification;
- single-use provider execution receipts for non-browser effects;
- exact durable authorization, actor, tenant, effect, payload digest, and
  idempotency validation;
- no direct database or credential sharing with Quarry.
...
### Build natively, under the correct owner
...
- action preview and deterministic verification;
- acquisition-driver/runtime/observation router;
- action cache;
- browser-procedure execution and promotion state;
- `SiteReliabilityProfile` and Adaptive Target Memory;
- traces, proof receipts, and artifacts;
- prompt-injection containment at the execution boundary;
- Quarry browser/acquisition benchmark harness.

**Model Plane / BrowserBroker:**

- planner/verifier model routing;
- browser grants and revocation;
- semantic replanning and bounded agent recovery;
- agent-level workflow/GraphPlan ownership.

**Application/Conversation and App Shell:**

- durable human approval/intent records;
- approval, replay, and takeover UX.

**Data Plane:**

- promoted source knowledge, chunks, embeddings, graphs, and retrieval assets.
...
3. repair levels.
4. site profiles.
5. browser-procedure compiler and promotion.
...
- every effectful action has an exact Control/Application/BrowserBroker authority chain, idempotency, and verification where required;
- repeated verified trajectories can be promoted into deterministic browser procedures;
...
```text
Control Plane
  canonical identity and exact organization membership

Application Plane / Conversation
  durable human intent and approval decisions
  notification and application delivery ledgers

Model Plane
  smart task planning
  planner/verifier model routing
  risk classification and approval requirements
  semantic verification and agent-level recovery

BrowserBroker
  exact grants, credential references, budgets, and revocation

Quarry V2
  search/fetch/browser acquisition routing
  browser observations
  safe typed action execution
  deterministic verification
  evidence, traces, proof receipts, and artifacts
  operational target memory and browser-procedure execution
  runtime/backend/observation routing

Integration / provider execution
  non-browser provider effects and authoritative provider receipts/read-back

Data Plane
  promoted source versions and durable knowledge assets

SolidJS App Shell
  live view, replay, approval/takeover UX, and proof display
...
Runtime backends
  ...
  Browserless
  Kernel
  future WebDriver BiDi-compatible browsers

Evaluation
  Quarry browser/acquisition Quality OS
  Model Plane model/agent evals
  shared BrowserGym/AgentLab/WebArena/WorkArena suites
```

Quarry V2 should become the **governed browser and web-intelligence execution engine** underneath Model Plane, not another end-to-end black-box browser agent, identity authority, approval authority, knowledge plane, or provider-integration plane.
...
| Maxun              | Human recorder to reusable robot, scheduled extraction                                                 | Browser-procedure recording UX donor           |
...
Quarry should borrow the recorder-to-compiled-browser-procedure UX. It should not embed
Maxun's platform because:

- it duplicates Quarry control and product surfaces;
- it is early-stage;
- its AGPL license requires careful legal review;
- Quarry already owns browser session execution, evidence, and audit receipts; Application/Conversation plus BrowserBroker own approval authority.
...
- compact procedure/action commands for compiled browser procedures;
...
## 42. Recorded browser procedure and deterministic script compiler
...
### 42.1 Sources of candidate browser procedures
...
recorded events
  ...
  -> security review
  -> candidate compiled browser procedure
  -> staged deployment
```

### 42.3 Compiled browser procedure contract

```ts
interface CompiledBrowserProcedure {
	procedureId: string;
	version: number;
	siteProfileId: string;
...
### 42.4 Replay policy and ownership boundary

A compiled browser procedure is a Quarry-owned, low-level deterministic
execution resource. It is not a Model Plane `GraphPlan`, business process, or
approval policy. Model Plane may select the procedure as a capability, but
Quarry alone resolves and executes its browser steps under a valid grant.

A compiled browser procedure may run without a planner only while:

- the site/profile compatibility signature matches;
- preconditions pass;
- target confidence exceeds the procedure threshold;
- risk and policy snapshots remain compatible;
- all required Application/Conversation approval references and BrowserBroker grants are valid;
- postconditions can be verified;
- failure rate remains below quarantine threshold.

Otherwise, re-observe and return a typed escalation to Model Plane rather than
blindly replay or silently convert the procedure into an agentic plan.
...
interface WebChangeRecord {
	...
	affectedBrowserProcedures: string[];
...
### 43.4 Browser-procedure impact analysis
...
capture new source version
  ...
  -> run impacted extraction/browser-procedure fixtures
  -> repair low-risk targets where allowed
  -> quarantine unsafe browser procedures
...
for acquisition, extraction, browser execution, deterministic recovery, and
proof generation. Planner-model quality remains owned by Model Plane; Quarry
records model identifiers and outcomes so the Model Plane eval system can score
and route them.
...
- p50/p95 latency;
- model-token metadata supplied by Model Plane;
- network/proxy/browser cost reported by Quarry;
- policy and security violations;
- browser-procedure quarantine rate;
...
Do not duplicate external effects for effectful browser procedures.
...
interface QuarryProofBundle {
	...
	policySnapshotId: string;
	controlAuthorityRef: string;
	applicationIntentOrApprovalRef?: string;
	browserGrantId?: string;
	authorizationPayloadDigest?: string;
	...
	cost: QuarryExecutionCostSummary; // browser/network/runtime only
	modelCostRef?: string; // owned by Model Plane cost-core
...
- Quarry Proof Bundle;
- browser-procedure recorder/compiler contracts;
...
3. Finish **Quarry execution continuation** for effectful browser work using
   the current run/step continuation model (confirmed greenfield, §18): persist
   the exact pending action and resume only after Application/Conversation has
   recorded the durable intent/approval and BrowserBroker has issued a matching
   tenant-/actor-/effect-bound grant. Do not add approval-decision authority to
   Quarry.
...
### P2: browser-procedure maintenance and quality

1. Record human takeover and verified agent trajectories.
2. Compile candidate deterministic browser procedures.
3. Add layout-mutation and dangerous-near-match tests.
4. Add browser-procedure impact analysis to `/v1/change`.
...
- automatic browser-procedure synthesis from many trajectories;
...
- a verified human or agent trajectory can become a reviewed deterministic
  browser procedure without granting Quarry planner, identity, or approval authority;
- a source change identifies impacted browser procedures and quarantines unsafe replay;
...
- anti-bot or challenge handling never overrides legal, robots, tenant, domain,
  identity, durable intent/approval, or BrowserBroker grant policy.
