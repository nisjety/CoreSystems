...
> | Deployment | **Quarry v2 Docker deployment wired into CoreSystem root `docker-compose.yml`**: 3 production-grade Dockerfiles (`Dockerfile.edge` Rust hot path, `Dockerfile.control` + `Dockerfile.orchestrator` Go services), `.dockerignore` strips `target/`, and root compose now boots `quarry-edge:8082` + `quarry-control:8081` + `quarry-orchestrator` (Temporal worker) reusing shared `aquatiq-postgres-local`/`aquatiq-redis-local`/`aquatiq-nats-local`/`org-core-temporal`. Replaces the V1 `quarry-api:8090` reference on the frontend. Bootstrap docs in `deploy/compose/README.md`. |
>
...
| 20 | **Stract + SearXNG Docker services in root compose** — add `stract:3000` (independent SERP index) and `searxng:8888` (long-tail aggregator) service blocks to `CoreSystem/docker-compose.yml`; both reuse `aquatiq-local` network; persistent volumes for stract index data; document in `deploy/compose/README.md` | Cycle 18 strategy (deployment integration) | Ops | 0.25 cycle | `docker compose up -d stract searxng` brings both services healthy; quarry-edge fallback chain reaches both |
