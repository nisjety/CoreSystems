...
RUNTIME_SECRET_KEYS=(
  ...
  MODEL_GATEWAY_ORG_CORE_SERVICE_TOKEN
...
preflight_release_inputs() {
  ...
  # Once the required gate locator is present, validate the external rollback
  # directory before opening any other release input. A missing or
  # non-canonical rollback artifact must be reported deterministically rather
  # than being masked by a later secure-input check, and no build may progress
  # toward Docker while the rollback binding is absent.
  rollback_artifact_directory >/dev/null
  secure_input_snapshot "$gates_source" public 65536 gates_file
