...
func (s *MemoryLeaseStore) Create(_ context.Context, scopeID, scopeType, orgID, ownerID, spaceID, backendID string, grant lease.SpaceGrant, ttl time.Duration) (*lease.Lease, error) {
	id, err := s.newID()
	if err != nil {
		return nil, err
	}
