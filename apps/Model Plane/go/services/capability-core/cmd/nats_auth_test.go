...
func TestNATSAuthOptionsUseCapabilityCoreInbox(t *testing.T) {
	tests := []struct {
		name         string
		user         string
		password     string
		...
		{name: "no credentials"},
		{name: "user password", user: "capability-core-runtime", password: "scoped-password", wantUser: "capability-core-runtime", wantPassword: "scoped-password"},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if options.InboxPrefix != "_INBOX.CAPABILITY_CORE_RUNTIME" {
				t.Fatalf("inbox prefix = %q, want capability-core-specific prefix", options.InboxPrefix)
			}
