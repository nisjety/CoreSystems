# Velion Chat — Competitive Parity Backlog (2026-08-01)

> Chat is the platform: every UI in Velion is an API the model can drive, powered by
> the model-gateway chat loop. This document maps what the leading AI chat/coding
> harnesses do best onto concrete parity work for Velion chat. The goal is not to
> copy but to reach parity where it makes Velion better.
>
...
> Velion already ships, then synthesized into a ranked backlog. Harnesses covered:
...
# VELION CHAT — PARITY BACKLOG

Synthesis of 9 harness reports + expert consensus into a ranked, deduplicated build plan. Every item is a thing Velion is missing or only partially has. Citations name the harness and source that establish the bar.

---

## 1. RANKED BACKLOG

Ranking weights: convergence across harnesses, whether it's a Velion self-declared gap, whether it unblocks other items, enterprise/strategic fit, and effort. Effort is S (days) / M (1–2 wk) / L (multi-week).
...
- **Why it matters:** This is Velion's own #1 known-weak item (transport buffers exist; SPA merge + producer detach unfinished). It is also the structural prerequisite for checkpoint/rewind, branch, edit-versions, and share links — build it once, four items unblock.
- **Velion coverage:** partial. Durable runs already do this (durable runId + live panel attach); ordinary chat turns do not.
- **Effort:** M.
...
- **Why it matters:** Seven of nine reports converge here. Lets Velion add domain capabilities (Norway data commons, Visma flows, shipping, compliance playbooks) at near-zero standing context cost, as versioned files instead of system-prompt bloat. Velion's existing "skill learning" is a Wilson-score *quality* loop — a different thing; it produces scores, not retrievable procedures.
- **Velion coverage:** partial (Wilson loop + RUN_COMPLETED→skill learning, but no skill *files*).
- **Effort:** M.
...
- **Why it matters:** Velion's explicit stated gap ("no subagent/parallel-agent orchestration in chat"). Quarantines verbose exploration so it never floods the main thread's budget. Deep research already has this shape — formalize it.
- **Velion coverage:** partial (durable RunAgent + parallel tool dispatch + agent console, but no chat-invokable fan-out).
- **Effort:** L.
...
- **Why it matters:** Grounding is Velion's core differentiator; Perplexity is the bar every reviewer measures against. Velion already computes citations + a graduated confidence score but *presents* rather than *gates* on it, and lacks the inline-chip UX. Cheap because the payload exists.
- **Velion coverage:** partial.
...
- **Why it matters:** Velion's stated gap. Makes bold multi-step agent actions low-risk. Note CC's own limitation — it does *not* track bash/side-effect mutations; Velion should scope checkpoints to reversible projections and mark provider writes as non-rewindable.
- **Velion coverage:** no.
- **Effort:** M (rides on #1).
...
- **Why it matters:** Editing a prompt after a bad answer is the single most common corrective gesture. Velion's stated gap. Velion *already tracks* edit-resubmit as a dissatisfaction signal — persisting the sibling instead of discarding it enriches that signal AND gives the switcher for free.
- **Velion coverage:** no.
- **Effort:** M.
...
- **Velion coverage:** partial (execution-core sandbox + file artifacts exist, but not a persistent per-run workspace the model reads/writes across steps).
- **Effort:** M.
...
- **Why it matters:** Surfacing a plan to the *user* (Velion has this via plan mode + run console) is not the same as reciting it back into the *model* each turn. Cheap, no architecture change, directly attacks goal-drift on ~50-call loops.
- **Velion coverage:** partial.
- **Effort:** S.
...
### #11 — Code-mode: Velion tools callable from inside the sandbox (collapse N tool rounds into one script)
- **Best-in-class:** Codex "code mode" (github.com/openai/codex code-mode/src/lib.rs); Hermes Python-RPC "zero-context-cost turns."
- **Why it matters:** Velion memory notes tools are dispatched sequentially and bounded by MAX_TOOL_ROUNDS. One authored script can loop/filter/join across many tools without each intermediate result re-entering context — big token + latency savings.
- **Velion coverage:** partial (run_code exists but can't call Velion's own tools).
- **Effort:** M.
...
- **Why it matters:** Velion's grounding is a read-side loop; there's no post-write verification, so the model can claim "done" without checking. Cuts hallucinated success.
- **Velion coverage:** partial.
- **Effort:** M.
...
- **Velion coverage:** partial (org/workspace + KB + context-packs, but no first-class project container scoping its own memory).
- **Effort:** L.
...
- **Why it matters:** Velion's memory is model-*written* (Dreaming loop). There's no explicit, deterministic, editable steering slot separate from inferred memory. Pairs with the shipped identity-context work (we/us→org, I/my→user): org-level instructions for "us," user-level for "I."
- **Velion coverage:** partial.
- **Effort:** S.
...
- **Why it matters:** Velion has the *heavy* path (deep research + live panel + durable runId) but no light inline variant for normal turns — casual readers see a clean answer, auditors expand. SSE tool-round events already exist.
- **Velion coverage:** partial.
- **Effort:** S.
- **First step:** When the multi-round tool loop runs, stream a collapsed summary block above the answer, expandable to the actual sub-queries/tool calls — a compact renderer over data Velion already streams.
...
- **Velion coverage:** partial (durable RunAgent + panel + HITL, but no completion notification binding).
- **Effort:** S.
...
- **Why it matters:** Loops are ~100:1 input:output; a one-token prefix change invalidates cache for everything after. Velion wires Anthropic prompt caching but the loop discipline is the other half (and the in-process prompt cache dies on deploy per memory).
- **Velion coverage:** partial.
...
- **Why it matters:** Complements Velion's semantic tiers (Dreaming/Letta) with exact-term recall ("the invoice number from that March thread") that embeddings miss.
- **Velion coverage:** partial (semantic memory yes; user-facing FTS over chat history no).
- **Effort:** S.
...
- **Why it matters:** Moves from "regenerate the whole artifact" to surgical edits. Velion already ships canvas/artifacts — this is an extension, not a new surface.
- **Velion coverage:** partial.
- **Effort:** M.
...
- **Velion coverage:** unclear (pin/filter exist; no evident public share).
- **Effort:** M.
...
- **Why it matters:** Velion measures cost per turn but doesn't systematically shape tool outputs. "Every UI is an API" gives a huge, confusable tool surface — the highest-ROI place to tune.
- **Velion coverage:** partial.
- **Effort:** M.
...
- **Velion coverage:** partial (panel is observe + approve, not take-the-wheel).
- **Effort:** L.
- **First step:** Where Velion drives a browser (Quarry-v2 / browser grants), add an interactive takeover mode streaming the live session to the panel, routing user input back into the same session, pausing the loop while the human acts.
...
- **Why it matters:** A provider/credential outage should fail over, not error the turn. Lower priority given Velion's SaaS model, but the failover half is worth mirroring.
- **Velion coverage:** partial (model ladder + Azure, no explicit rotation).
- **Effort:** S.
...
1. **#1 Server-authoritative sessions + stream resume (M).** It's Velion's own #1 gap *and* the load-bearing foundation: checkpoint/rewind (#7), branch/edit-versions (#8), and share links (#22) all sit on top of it. Reuses the durable-run attach model Velion already proved. Build this first or the four downstream items each reinvent persistence.

2. **#2 Declarative hook + permission-policy layer (M).** Directly fixes the recurring "HITL decorative on live writes" landmine that appears across Velion's own audit memory. It's the single biggest enterprise-trust differentiator in the whole backlog — Codex/Claude Code/OpenCode all treat this as core, and Velion's multi-tenant + ZDR posture makes it more valuable here than anywhere. Converts model cooperation into a hard boundary.
...
4. **#5 Inline citation chips + quality gate (S/M).** Defends Velion's *core* strategic differentiator (grounded, cited answers) against the acknowledged bar (Perplexity) — cheaply, because the citation + confidence payload already exists. High visible-quality-per-effort. The abstain-and-requery gate prevents the one failure mode a cited-answer product cannot afford: confidently wrong.

5. **#4 In-chat subagent fan-out (L).** Velion's explicit stated gap and the frontier capability that separates "chat" from "agent platform." Higher effort, but reuses durable RunAgent + parallel dispatch. Scope v1 to read-only research subagents to stay on the safe side of the Cognition/Anthropic multi-agent split.
...
## 3. WHAT VELION ALREADY DOES AS WELL AS / BETTER THAN THE FIELD — DO NOT REGRESS

Confirmed by the reports' own "what_velion_does_better" sections across all 9 harnesses:

- **First-class grounded retrieval** — DP retrieval + GraphRAG + RRF hybrid + rerank + citations + **graduated confidence score** over governed multi-tenant data. *No* competing harness has a first-class citation/confidence/knowledge-graph-traversal layer; Perplexity is web-first with no graph arm and no exposed confidence gate. This is Velion's moat.
- **Cost-aware model routing** — intent×budget → velion-budget/balance/genius ladder + real BPE tokenizer + per-turn cost/token badge. Every harness only *preaches* token economics (Manus/consensus); Velion operationalizes them. Perplexity/ChatGPT/Manus hide cost entirely.
...
- **Native business-action surface** — Visma MCP, shipping, provider actions, multi-channel inbox, image/xlsx/docx/pdf. Code harnesses act on a repo; Velion acts on real business systems.
