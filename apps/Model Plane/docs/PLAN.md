...
5. **Cost / analytics.** `Model Plane v2` ships a dedicated `cost-core` (run_costs, analytics_events, NATS `velion.cost.*`); fork ships `services/analytics/`. Model Plane has neither. `model-gateway` does not emit `usage.{org}.llm` events.
