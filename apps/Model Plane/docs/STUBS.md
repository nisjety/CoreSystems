...
| **Sandbox manager** (seam) | `go/services/sandbox-manager` | Lease + snapshot **bookkeeping is real + deployed** (`AcquireLease`/`ReleaseLease`, TTL/expiry; gateway holds a `sandbox_client`); only sandbox **provisioning** (what a lease's endpoint points to) is unbuilt. **RULING (2026-06-03) — decided, not deferred:** a lease is scoped to a *thread/agent with TTL+endpoint* = the **per-session** sandbox model (daytona-parity), which is **complementary, NOT a duplicate** of the executor's **per-call** bwrap isolation (codex-parity) — different granularities, different jobs (correcting an earlier mischaracterization). Per the directive ("small but powerful, not over-engineered"): per-call isolation (executor) is **canonical + sufficient today**; full provisioning is **YAGNI-deferred** — the `sandbox_client` is wired but **no caller leases a sandbox**, so building a container/VM lifecycle manager now would be over-engineering power with no job. Keep the thin lease seam; build provisioning only when a concrete per-session-workspace consumer drives it (needs a runtime to verify). |
