...
- 🔒 **Boundary verified by investigation (2026-05-30) — why the remaining consolidation is genuinely stack-gated, not a missed quick win.** Two candidates were code-checked and confirmed blocked (so building them now would be unverifiable/speculative — the over-engineering the directive forbids):
  - **§4.3 read-path consumer:** the gateway has **zero NATS subscriber infrastructure** (`model-gateway/src` has a publisher only — no `subscribe`/jetstream/consumer anywhere). So the consumer isn't a small handler add — it's net-new subscription wiring (connect→subscribe→dispatch) that cannot run or be verified without a live bus, and a pure decode-handler with no subscriber to call it would be dead code. Correctly deferred to the running pair.
