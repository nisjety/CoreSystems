...
- 🟡 **G7 producer core + activity logic** — `capability-core/internal/learning`: retention policy (`SelectForPersistence`: provenance protection, dedup, threshold) **and** the activity control flow (`RunReview` over injected `Reviewer`/`Sink` interfaces); MIT-adapted prompt; 12 tests, `go vet` clean.
  - **Sink ownership RESOLVED (data-model, 2026-05-30):** a `SkillCandidate` carries a *body* (content, trigger_keywords, tool_restrictions) — which matches **session-core's `agent_skills` table exactly**, and does **not** fit capability-core's `Capability` (catalog metadata only: ID/Name/Kind/Scope — no content field). So **skill bodies live in session-core `agent_skills`; capability-core's registry is the catalog/index over them.** The concrete `Sink` must call a session-core **`UpsertAgentSkill` RPC (does not exist yet)** — an *additive* new RPC + handler (the table exists). This is the cross-service seam; plus the concrete `Reviewer` (inference-core LLM) and the Temporal session-end trigger. Building the RPC is additive/safe (can't regress current behavior) but the durable write + LLM call need the stack to verify.
