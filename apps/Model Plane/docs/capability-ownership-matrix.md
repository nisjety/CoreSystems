...
- **2026-05-30 — implementation path (verified):** capability-core has **no publisher wired** today (only an unrelated SSE `streaming.Streamer`); the shared **`go/pkg/publisher`** (`EventPublisher` iface + `NATSPublisher` + an **`InMemoryPublisher` with `Drain()` for tests**) is ready to inject. Mutation points to emit on: `registry.ModelsRegistry.Upsert`, `registry.CapabilitiesStore.Upsert`, and the `*Handler.listOrCreate` HTTP handlers (Skills/MCP/Routing/Safety). **Emission is unit-testable now** (inject `InMemoryPublisher`, assert subject+payload on `Drain()`); the **gateway-subscribe half + live NATS delivery are runtime-bound**. Not built here as an isolated emitter would be an uncalled primitive — it should land *with* its publisher injection through `cmd/main.go` so it's wired end-to-end, reviewed against the running gateway cache.
