...
- 🟡 **G7 producer core** — `capability-core/internal/learning` (SkillCandidate DTO, provenance enum, content-hash dedup, **user-skill protection**, confidence threshold, MIT-adapted review prompt; 6 tests, `go vet` clean). The LLM call + Temporal post-session activity + registry persistence are the thin seam on top (need stack).
- ⬜ **G8** — pending (gateway↔backend durability wiring; needs stack).
- ⚠️ **P8 finding (correctness):** `mp-toon` is **encode-only AND lossy by construction** — unquoted scalars conflate types (the string `"30"`, the number `30`, and would-be bools all emit bare `30`), so `encode` is not injective and **no decoder can losslessly invert it**. Consequences: (1) the roadmap's "TOON round-trip/reversibility gate" is **unachievable as designed**; (2) TOON is safe ONLY for prompt rendering / display, and must **never** be used on a path that round-trips data or could replace a source record (GOAL.md non-negotiable: correctness/traceability win). **To make it reversible** the encoder must type-tag scalars (quote all strings, or prefix types) — a format change, not just a decoder. Documented here so no one builds a false-confidence `decode`. The module doc's "losslessly round-trippable" claim is **inaccurate** and should be corrected.
