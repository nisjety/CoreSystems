...
async fn deny_mode_blocks_execution() {
    let outcome = runtime_loop::execute_step("echo", "payload", "deny", "", "", "", "", "", None, None).await;
...
async fn ask_mode_requires_approval_for_tools() {
    ...
    let outcome =
        runtime_loop::execute_step("delete_account", "payload", "ask", "", "", "", "", "", None, None).await;
    assert_eq!(outcome.status, "awaiting_approval");
}

#[tokio::test]
async fn auto_mode_executes_tool() {
    let outcome = runtime_loop::execute_step("echo", "hello", "auto", "", "", "", "", "", None, None).await;
...
async fn hook_can_block_step() {
    ...
    let outcome =
        runtime_loop::execute_step("echo", "hello", "auto", hook_context, "", "", "", "", None, None).await;
...
async fn fail_tool_returns_error() {
    let outcome = runtime_loop::execute_step("fail", "payload", "auto", "", "", "", "", "", None, None).await;
    assert_eq!(outcome.status, "failed");
    assert!(!outcome.error.is_empty());
}
...
async fn subagent_spawn_appends_summary() {
    let outcome =
        runtime_loop::execute_step("subagent.xyz", "payload", "auto", "", "", "", "", "", None, None).await;
    assert_eq!(outcome.status, "completed");
...
async fn compaction_triggers_when_output_large() {
    ...
    let outcome = runtime_loop::execute_step("echo", &big, "auto", "", "", "", "", "", None, None).await;
    assert_eq!(outcome.status, "completed");
...
async fn reasoning_step_without_tool() {
    let outcome = runtime_loop::execute_step("", "payload", "auto", "", "", "", "", "", None, None).await;
    assert_eq!(outcome.status, "completed");
