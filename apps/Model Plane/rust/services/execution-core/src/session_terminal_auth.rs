//! Fixed-scope service credentials for managed-run terminalization.
//!
//! Execution Core must not reuse its Quarry or capability-policy credentials:
//! terminal outcomes use the distinct `aud=session-core` trust boundary.
...
impl TerminalScope {
    ...
    const fn reason_suffix(self) -> &'static str {
        match self {
...
/// Mints short-lived, tenant-bound service JWTs for the two terminal APIs.
#[derive(Clone)]
...
/// Narrow capability used by the agent driver. Production delegates to the
/// fixed-scope Auth Core minting client; focused runtime tests can supply a
/// deterministic fake without ever accepting a delegated user credential as a
/// terminalizer.
#[tonic::async_trait]
pub(crate) trait ManagedRunTokenProvider: Send + Sync {
    ...
    async fn mint(&self, key: &CacheKey) -> Result<TokenResponse, SessionTerminalTokenError> {
        let mut credential =
            reqwest::header::HeaderValue::from_str(&self.credential).map_err(|_| {
                SessionTerminalTokenError::Configuration("EXECUTION_CORE_SERVICE_API_KEY")
            })?;
        ...
        let response = self
            ...
            .json(&serde_json::json!({
                ...
                "reason": format!("{} {}", self.service_id, key.scope.reason_suffix()),
            }))
            .send()
            .await?;
...
#[tonic::async_trait]
impl ManagedRunTokenProvider for SessionTerminalTokenProvider {
    async fn terminalize_token(&self, org_id: &str) -> Result<String, SessionTerminalTokenError> {
        SessionTerminalTokenProvider::terminalize_token(self, org_id).await
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn mints_only_the_fixed_execution_terminal_and_heartbeat_scopes() {
        ...
        Mock::given(method("POST"))
            ...
            .and(body_json(serde_json::json!({
                ...
                "reason": "execution-core managed-run terminalization"
            })))
    ...
    async fn rejects_a_non_session_core_token_response() {
        ...
        Mock::given(method("POST"))
            ...
            .await;
        let provider =
            SessionTerminalTokenProvider::new_for_test(auth.uri(), "execution-core", "secret");
