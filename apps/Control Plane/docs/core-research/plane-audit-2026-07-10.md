...
| `ENVIRONMENT_FILES.md` | **update** | Edited in place: corrected stale `coresystem-*-local` hostnames to real container names, replaced Redis references with Dragonfly, added billing-core/session-core/audit-core to the port-mapping table (previously only auth/user/org). |
