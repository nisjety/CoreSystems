...
- Verevon/Data real-authority proof passes 31 gRPC methods across four auth shapes (124 checks), 28/28 HTTP checks, real Auth/User integration, and 2/2 browser E2E cases. Gateway Rust evidence remains 288 tests with all selected security modules above 80%.
...
exhausted deliveries to `verevon.gdpr.erasure.dlq.documents-api` with a valid
...
administer JetStream. `verevon.gdpr.*` is explicitly excluded from the legacy
token bridge.
...
| Verevon v3 gateway/SPA         | **Healthy; both rebuilt from current worktree**                          | Gateway `/health` and SPA `/` return 200. Callback, org, membership, billing, session, logging, and audit contract tests pass. The local SPA is the dev target; release nginx syntax is separately verified.                                                                                                                                                                                                                      |
...
audit-core-service=starting; verevon-gateway-rs=unhealthy
```
...
| Verevon v3 SPA  | ESLint (0 errors, 1 pre-existing warning), typecheck, production build, 68 files/358 tests pass                                                                                                                                                                                        | Existing focused changed-module measurements remain recorded below; no whole-SPA percentage is claimed.                                                                                                                                                                                               |
| Verevon gateway | format, debug/release workspace checks, strict Clippy (`-D warnings`), all 288 Rust tests, and 4/4 coverage-gate contract tests pass                                                                                                                                                   | audience tokens 98.82%; config 90.63%; membership boundary 89.62%; middleware 87.32%; upstream 81.55%.                                                                                                                                                                                                |
