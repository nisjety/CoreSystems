...
// already uses that name for something unrelated. See
// migrations/028_org_group_grants.sql for the full rationale.
export const orgGroup = pgTable(
  ...
  (table) => [
    uniqueIndex('org_group_host_unique').on(table.hostOrganizationId),
  ],
);
...
export const orgGroupGrant = pgTable(
  ...
  (table) => [
    uniqueIndex('org_group_grant_unique').on(
      table.orgGroupId,
      table.organizationId,
    ),
    check(
