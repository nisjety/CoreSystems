...
async function createAdminUser() {
  ...
  try {
    ...
    if (signUpResult.success || signUpResult.error?.includes?.('already exists')) {
      ...
      if (signInResult.success) {
        ...
        console.log('💡 This is normal if HIBP (password breach check) is enabled');
      }
    } else {
...
createAdminUser().then(() => {
  ...
  console.log('3. Microsoft OAuth URL: http://localhost:3011/api/auth/sign-in/microsoft');
  process.exit(0);
}).catch(error => {
