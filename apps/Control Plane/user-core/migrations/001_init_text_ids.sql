-- User Service Database Schema (TEXT IDs for Better Auth compatibility)
-- Modified from 001_init.up.sql to use TEXT instead of UUID
...
-- Insert default roles (using simple incrementing IDs since we're using TEXT)
INSERT INTO roles (id, name, description, permissions) VALUES
    ('role-1', 'admin', 'Administrator with full access', '["*"]'::jsonb),
    ('role-2', 'user', 'Regular user', '["read:own", "update:own"]'::jsonb),
    ('role-3', 'moderator', 'Content moderator', '["read:all", "update:content", "delete:content"]'::jsonb)
