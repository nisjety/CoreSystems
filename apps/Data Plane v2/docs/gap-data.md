...
No items currently rated `high` priority remain open. Wave-3 candidates: ~~`wiki_block_embeddings` write-through (vs. just collection ensure)~~ (**DONE** — `wiki-store-go` enqueues `dataplane.wiki.version.published` to its durable outbox in `internal/repo/wiki_repo.go`, `internal/events/outbox.go` publishes it with PubAck, and `embedding-engine-rs/src/wiki_consumer.rs` embeds and upserts the point on a durable consumer); ~~4-way scalar blend that actually consumes graph+wiki signals at scoring time~~ (**graph DONE** — the GraphRAG program folds `w_graph` into RRF via the graph arm; wiki was already fused via `w_wiki`; see `docs/graphrag-neo4j-plan.md`); tree-sitter swap-in for code AST.
...
- **Open (wave 3+, none rated `high`)**: nightly entity summarizer → `entity_summary_embeddings`; full 4-way scoring with graph + wiki signals folded in; tree-sitter swap-in for code AST; live-traffic measurement against the 800ms gate; JWKS endpoint fetch (currently static PEM only).
...
| 6 | **§16.3.8 wiki_block_embeddings write-through missing** | New `services/wiki-store-go/internal/events/publisher.go` with `PublishWikiVersionPublished` event (`dataplane.wiki.version.published`). go.mod adds `nats.go v1.39.1`. Wiring into `CreatePage` / `CreateVersion` + embedding-engine subscriber are the next steps — the publisher contract is locked in. **2026-08-25 update: fully CLOSED** — those next steps landed: `internal/repo/wiki_repo.go` enqueues `dataplane.wiki.version.published` into the durable outbox, `internal/events/outbox.go` drains it with PubAck, and `embedding-engine-rs/src/wiki_consumer.rs` embeds and upserts into `wiki_block_embeddings` on a durable consumer. | `services/wiki-store-go/internal/events/publisher.go`, `go.mod` |
