...
// fakeDelivery, fakeDLQPublisher, and validDeliveryPayload live in
// durable_consumer_test.go (same package) — reused here so the org-purge
// consumer's delivery-lifecycle tests exercise the exact same fakes as the
// per-user ownership-transfer consumer's, keeping the two suites comparable.
...
func TestProcessOrgPurgeDeliveryMovesPoisonToDLQBeforeACK(t *testing.T) {
	...
	if delivery.acks != 1 || dlq.calls != 1 || dlq.msg == nil || dlq.msg.Subject != orgPurgeDLQSubject {
...
func TestStartOrgPurgeSubscriberBindsPreprovisionedDurableACKsAndReportsHealth(t *testing.T) {
	...
	if _, err := js.AddStream(&nats.StreamConfig{
		...
		Subjects: []string{
			ErasureRequestedSubject, orgPurgeDLQSubject,
		},
	...
	if _, err := js.AddConsumer(controlSharedStream, &nats.ConsumerConfig{
		Durable: orgPurgeDurableConsumerName, DeliverSubject: orgPurgeDurableDeliverySubject,
		DeliverGroup: orgPurgeDurableConsumerName, FilterSubject: ErasureRequestedSubject,
