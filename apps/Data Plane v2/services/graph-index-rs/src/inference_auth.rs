...
//!
//! Graph extraction uses the same audited service-principal contract as the
//! retrieval embedding path. A shared API key is never forwarded to gRPC; the
//! short-lived bearer is minted for the exact organization being processed and
//! is bounded before it reaches inference-core.
...
const TOKEN_REASON: &str = "extract graph entities";
