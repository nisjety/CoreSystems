...
func TestClaimsHasScopeRequiresVerifiedIdentity(t *testing.T) {
	if (*Claims)(nil).HasScope("data:orchestrator:write") {
		t.Fatal("nil claims must not authorize a scope")
	}
	unverified := &Claims{Scopes: []string{"data:orchestrator:write"}}
	if unverified.HasScope("data:orchestrator:write") {
		t.Fatal("unverified claims must not authorize a scope")
	}
	verified := &Claims{Verified: true, Scopes: []string{"data:orchestrator:read", "data:orchestrator:write"}}
	if !verified.HasScope("data:orchestrator:write") || verified.HasScope("data:orchestrator:admin") {
		t.Fatal("verified scope matching must be exact")
	}
}

func TestRequireAnyScopeFailsClosedAndAcceptsAnExplicitCapability(t *testing.T) {
	next := http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusNoContent)
	})
	handler := RequireAnyScope("data:orchestrator:write", "data:orchestrator:admin")(next)
	...
	tests := []struct {
		...
		{name: "missing verified identity", wantStatus: http.StatusUnauthorized},
		{name: "wrong scope", claims: &Claims{Verified: true, Scopes: []string{"data:orchestrator:read"}}, wantStatus: http.StatusForbidden},
		{name: "accepted write scope", claims: &Claims{Verified: true, Scopes: []string{"data:orchestrator:write"}}, wantStatus: http.StatusNoContent},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodPost, "/v1/orchestrator/jobs", nil)
			if tt.claims != nil {
				req = req.WithContext(context.WithValue(req.Context(), contextKey{}, &requestIdentity{claims: tt.claims}))
			}
			response := httptest.NewRecorder()
			handler.ServeHTTP(response, req)
			if response.Code != tt.wantStatus {
...
// unattended orchestration impossible: stale-embedding sweeps and
// reindex jobs are precisely the callers that hold a service credential
