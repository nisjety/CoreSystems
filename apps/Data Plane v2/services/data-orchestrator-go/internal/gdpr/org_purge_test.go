...
// A per-user erasure/anonymize fan-out arrives on the exact same subject.
// HandleOrgErasure must skip it silently — no purge call, no error, no
// DLQ-worthy poison — since it is a well-formed event this consumer simply
// doesn't own. This is the safety-critical gate from the task brief: never
// let a per-user event trigger an org-wide purge.
func TestHandleOrgErasureSkipsNonOrganizationSubjects(t *testing.T) {
	for _, payload := range []string{
		`{"event_id":"gdpr:fanout:child-1","subject_type":"user","subject_id":"user-a","org_id":"org-1","requested_by":"admin","ts":"2026-07-20T00:00:00Z"}`,
		`{"subject_type":"user_anonymize","subject_id":"user-a","org_id":"org-1","requested_by":"admin","ts":"2026-07-20T00:00:00Z"}`,
