...
- **Chat's auto-attached Brreg lookup tool is silently broken, found 2026-08-02** (`src/features/chat/lib/brreg-action.ts`): its action id is spelled `brreg_lookup_organization` (underscore) but the real registry id is `brreg.lookup_organization` (dot). The id mismatch means `createSelectedAgentToolSpecs` never finds the real parameter schema and falls back to an empty-params stub — every time chat auto-attaches this action (any Brreg-related keyword in the user's message), the model receives a tool that looks usable but declares zero parameters for a lookup that requires a query string. One-line id fix.
...
- **Real, live, user-facing bug found 2026-08-02**: the social calendar's notion of "today" and every new draft's schedule time are hardcoded to a frozen `2026-06-15`/`2026-06-17` test-fixture date (`SocialCalendarPage.tsx`), not the real clock — it leaked from `social-calendar.test.ts`'s deterministic fixture into production code. Today is 2026-08-02: the calendar never highlights the actual day, always opens on a stale June 2026 month, and **every draft a user creates today is silently scheduled about two months in the past.** Trivial fix (default to `new Date()`), high user-visible impact until fixed.
- **The registry's single highest-risk action bypasses its own approval gate.** `social.publish_post` is declared `risk: high, requiresApproval: true, reversible: false` in `action-registry.ts`, but the human UI (`SocialCalendarPage.tsx`) never calls it through the `executeAction` dispatcher — it calls `publishSocialPost()` directly against the REST route, which has no `approvalId` parameter at all and no client-side check that a post was actually approved before the publish button is clickable. Same bypass pattern as ticketing (above), but on the one social action explicitly marked irreversible. A second, related drift: the gateway implements social schedule/publish TWICE (the UI's direct REST path in `domains/social.rs`, and the agent-facing dispatcher in `dispatchers.rs`) and the two have diverged — the agent path skips the RFC3339 timestamp validation the UI path has.
...
- **Security-relevant regression, found 2026-08-02**: the 2FA step-up re-authentication password field renders as `type="text"` — the account password is shown in plaintext while the user types it, with no password-manager treatment. `SettingsField`'s `type` union doesn't even offer `'password'` as an option, so this isn't a one-line typo, the component needs extending. The identical step-up flow in `PrivacyDataSection.tsx` correctly uses `type="password"`. Fix ahead of any demo involving 2FA.
...
- **Cross-tenant approval IDOR, found 2026-08-02** (code-health audit of `apps/gateway/src/domains/orchestration.rs`, self-flagged in its own doc comment): model-gateway's `GetApprovalRequest`/`DecideApprovalRequest` carry no `org_id`, so the gateway's approval proxy — despite correctly verifying the caller's session — cannot enforce org-ownership on an approval beyond what model-gateway checks (today, nothing). Any authenticated user of the platform, in any org, who knows/guesses an `approval_id` can view or decide another org's pending HITL approval. This is a sharper, distinct gap from the already-known "approval gRPC surface is unauthenticated" item — it means even a *fully authenticated* caller crosses tenant boundaries here. Fix belongs in session-core/model-gateway (stamp and check `org_id` on the approval record), not in the gateway proxy.
...
- **A real navigation bug in the Run Console, found 2026-08-02**: `AgentsProvider` runs its own `window.history`/`popstate` channel in parallel with `@solidjs/router`, but is mounted once at the app-shell root instead of per-route — an ordinary in-app client-side nav (e.g. select the Service role, visit Inbox, click the sidebar's bare "Agents" link) fires no `popstate`, so the stale role selection persists and the overview never shows even though the URL is bare `/agents`. Existing tests can't catch this because every test constructs a fresh provider per render. `AgentRunConsole.tsx` itself — the one file that owns all of this feature's real business logic (HITL decide/resume/cancel, SSE timeline dedup) — is 1,782 lines with **zero tests**.
...
8. **New, 2026-08-02 — three CRITICAL-severity findings from a full down-the-stack code-health audit.** Full detail in `velion-roadmap.md` §3a; summarized here because they materially change the trust picture: (a) **Quarry-v2's headless-browser driver performs zero SSRF enforcement of its own** — its own test suite (`quarry-browser/tests/ssrf.rs`) asserts a security contract the implementation doesn't satisfy, and every test in that file is permanently `#[ignore]`d, so nobody would notice; this directly undercuts CLAUDE.md's rule that Quarry-v2 is the trusted arbiter of browser actions. (b) **The static crawl driver auto-follows redirects with no SSRF re-check** — a 301/302 to a private/metadata address is followed transparently. (c) **social-core's actual publish path bypasses integration-corev2's actions surface entirely for every provider write** (LinkedIn/Meta/TikTok/Snapchat) — a live, currently-wired violation of the single architecture rule this monorepo states most explicitly ("Provider actions go through integration-corev2's actions surface"), not a stale or theoretical one. A fourth, related HIGH finding: the same DNS-rebinding TOCTOU gap (resolve-then-discard, re-resolved independently at connect time) exists in BOTH Quarry-v2's Rust guard and imports-core's Python guard — a systemic pattern, not a one-off.
