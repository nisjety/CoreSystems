...
**What verifiably exists**
- `/agents/runs` **Agent Run Console is real**: runs list/detail, SSE timeline, plans/todos/lineage via orchestration-client. Server-side HITL is real and enforced (execution-core pauses risky tools pre-execution; durable approvals in session-core). Durable Temporal workflows exist (deep task, wide research, memory consolidation, skill promotion). Cost (`/agents/cost`) and eval (`/agents/quality`) pages wired.
...
- Post-approval continuation P0 (see 1.2). Cost ledger in-memory. Capability catalog: all 27 rows derive *unavailable* pending health attestation. Sandbox/browser-broker stores in-memory.
